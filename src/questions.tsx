export const Questions = {
  savingAccount: [
    {
      question: "Platím zpravidla za vedení spořícího účtu poplatky?",
      options: ["Ano", "Ne"],
      answer: "Ne",
    },
    {
      question: "mohu si zpravidla kdykoliv vybrat peníze ze spořicího účtu?",
      options: ["Ano", "Ne"],
      answer: "Ano",
    },
    {
      question: "Mohu si zpravidla kdykoliv vložit peníze na spořící účet?",
      options: ["Ano", "Ne"],
      answer: "Ano",
    },
    {
      question: "Kolik % Čechů má spořící účet",
      options: ["20%", "40%", "60%", "80%"],
      answer: "60%",
    },
    {
      question: "Co jsou tzv. úroková pásma?",
      options: [
        "Úrokové sazby, které se mění v závislosti na výši vkladu",
        "Úrokové sazby, které se mění v závislosti na době vkladu",
        "Úrokové sazby, které se mění v závislosti na výši úrokové míry",
        "Úrokové sazby, které se mění v závislosti na výši inflace",
      ],
      answer: "Úrokové sazby, které se mění v závislosti na výši vkladu",
    },
    {
      question: "Můžu ze spořícího účtu přímo platit, například v obchodě?",
      options: ["Ano", "Ne"],
      answer: "Ne",
    },
    {
      question: "Co dělá primárně banka s penězi z účtů?",
      options: [
        "Půjčuje je dalším lidem",
        "Investuje je na trzích",
        "Ukládá je u ČNB",
        "Utratí je",
      ],
      answer: "Ukládá je u ČNB",
    },
    {
      question: "Co je to dvoutýdenní repo sazba?",
      options: [
        "Úroková sazba, za kterou si centrální banka od komerčních bank půjčuje peníze",
        "Úroková sazba, za kterou si komerční banky půjčují peníze od centrální banky",
        "Úroková sazba, za kterou si komerční banky půjčují peníze od sebe",
        "Úroková sazba, za kterou si klienti půjčují peníze od centrální banky",
      ],
      answer:
        "Úroková sazba, za kterou si centrální banka od komerčních bank půjčuje peníze",
    },
    {
      question: "Jakou likviditu lze očekávat od spořících účtů?",
      options: ["Okamžitou", "Vysokou", "Střední", "Nízkou"],
      answer: "Vysokou",
    },
    {
      question:
        "Jaký přibližný výnos lze očekávat od spořících účtů při 3% inflaci?",
      options: ["1-2%", "2-3%", "3-4%", "4-5%"],
      answer: "1-2%",
    },
    {
      question: "Je repo sazba větší než úroková míra na spořícím účtu?",
      options: ["Ano", "Ne"],
      answer: "Ano",
    },
    {
      question:
        "V roce 2024 byla v prvním čtvrtletí inflace 3%, i tak byly výnosy z úroků na spořicích účtech větší než inflace (4-5%). Jak je to možné?",
      options: [
        "Inflace byla v předchozích dvou letech velmi vysoká, úroky na spořicím účtu reagují se zpožděním",
        "Banky potřebovaly získat klienty, takže nabídly vyšší úrokové sazby",
        "Úroky na spořicím účtu jsou běžně vyšší než inflace",
        "Spořící účty jsou závislé na jiných faktorech než inflace",
      ],
      answer:
        "Inflace byla v předchozích dvou letech velmi vysoká, úroky na spořicím účtu reagují se zpožděním",
    },
    {
      question:
        "Do jaké částky je pojištěný vklad na spořicích a běžných účtech u jedné banky",
      options: ["1 000 000 Kč", "50 000 €", "2 000 000 Kč", "100 000 €"],
      answer: "100 000 €",
    },
    {
      question: "Snižuje se úrok na spořících účtech s vloženou částkou?",
      options: [
        "Ano, přímo úměrně vložené částce",
        "Ano, úměrně smluvně daným úrokovým pásmům",
        "Ne, úrok je vždy stejný",
        "Ne, úrok se zvyšuje s vloženou částkou",
      ],
      answer: "Ano, úměrně smluvně daným úrokovým pásmům",
    },
    {
      question:
        "Jaký lze zpravidla očekávat úrok u vyšších částek (např. nad 500 000 Kč)?",
      options: ["0,1%", "0,5%", "1%", "1,5%"],
      answer: "0,1%",
    },
    {
      question:
        "Na spořící účet si vložím 2 000 000 Kč. Základní úrok je 2%, od 500 000 korun klesá na 0,1%. Jaký výnos získám za rok (bez daně)?",
      options: ["2 000 Kč", "11 500 Kč", "22 500 Kč", "40 000 Kč"],
      answer: "11 500 Kč",
    },
    {
      question: "Je zpravidla povinný minimální vklad na spořící účet?",
      options: ["Ano", "Ne"],
      answer: "Ne",
    },
    {
      question: "Pro tvorbu jakého typu rezervy je spořící účet vhodný?",
      options: [
        "Rezervy na nečekané krátkodobé výdaje",
        "Dlouhodobé finanční rezervy",
        "Rezervy na důchod",
        "Rezervy na investice",
      ],
      answer: "Rezervy na nečekané krátkodobé výdaje",
    },
    {
      question: "Na co se zpravidla využívá krátkodobá rezerva?",
      options: [
        "Drobnější opravy domu či auta, náhrada domácích spotřebičů",
        "Nákup nemovitosti",
        "Výbava bytu a jeho celková rekonstrukce",
        "Dovolená",
      ],
      answer: "Drobnější opravy domu či auta, náhrada domácích spotřebičů",
    },
    {
      question:
        "Jaký je zpravidla doporučený objem krátkodobé rezervy v závislosti na výdajích daného člověka?",
      options: [
        "1-2 měsíčních výdajů",
        "3-6 měsíčních výdajů",
        "9-12 měsíčních výdajů",
        "1-2 ročních výdajů",
      ],
      answer: "3-6 měsíčních výdajů",
    },
    {
      question:
        "Za jak dlouhou dobu je možné získat peníze ze spořícího účtu v podobě hotovosti ?",
      options: ["Okamžitě", "Do 1 dne", "Do 3 dnů", "Do 1 týdne"],
      answer: "Okamžitě",
    },
    {
      question:
        "Pro financování menší havárie v koupelně (např. rozbitý bojler) je vhodné využít:",
      options: [
        "Prostředky z akciového fondu investované do něj před 2 lety",
        "Prostředky ze stavebního spoření, založeného před 4 lety",
        "Prostředky, které má klient na spořicím účtu",
        "Prostředky z pětiletého termínovaného vkladu vložené před třemi lety",
      ],
      answer: "Prostředky, které má klient na spořicím účtu",
    },
    {
      question:
        "Finanční poradce doporučí klientovi, aby pohotová finanční rezerva, zůstatek na běžném či spořicím účtu, kterým může klient krýt výpadek příjmů, byla alespoň:",
      options: [
        "Přibližně 6× průměrného příjmu",
        "Přibližně 12× průměrného příjmu",
        "Přibližně 18× násobek průměrného příjmu",
        "Přibližně 24× násobek průměrného příjmu",
      ],
      answer: "Přibližně 6× průměrného příjmu",
    },
    {
      question: "Jaký je rozdíl mezi spořením a investováním?",
      options: [
        "Při investování vystavuji peníze riziku ztráty, u spoření ne",
        "Spoření a investování mají stejný význam",
        "Investování je jen pro bohaté, spoření pro běžné lidi",
        "Spoření je více ovlivněno krizemi finančního trhu",
      ],
      answer: "Při investování vystavuji peníze riziku ztráty, u spoření ne",
    },
    {
      question:
        "Tomáš uložil na spořící účet 4 000 000 korun, proč to není nejlepší rozhodnutí?",
      options: [
        "Zhodnocení vysoké částky je mizerné, vklad je pojištěný jen do 100 000 €",
        "Spořící účet není bezpečný",
        "Více peněz na účtu znamená vyšší poplatky",
        "Peníze ze spořícího účtu je těžké vybrat",
      ],
      answer:
        "Zhodnocení vysoké částky je mizerné, vklad je pojištěný jen do 100 000 €",
    },
    {
      question: "Jak může vypadat podmíněný úrok na spořícím účtu?",
      options: [
        "Takovou možnost banky nenabízí",
        "Klient musí několikrát za měsíc použít kartu běžného účtu stejné banky",
        "Klient musí mít na účtu minimální zůstatek",
        "Klient musí vložit minimální částku",
      ],
      answer:
        "Klient musí několikrát za měsíc použít kartu běžného účtu stejné banky",
    },
    {
      question: "Peníze se na spořícím účtu zpravidla neúročí: ",
      options: ["Denně", "Týdně", "Měsíčně", "Čtvrtletně"],
      answer: "Týdně",
    },
    {
      question: "Jak se dá vyhnout snižování úroky vlivem úrokových pásem?",
      options: [
        "Vkládat peníze postupně",
        "Vkládat peníze najednou",
        "Rozložit peníze na účty u více bank",
        "Snižování úroků se nelze vyhnout",
      ],
      answer: "Rozložit peníze na účty u více bank",
    },
    {
      question: "Můžu vybrat peníze ze spořícího účtu z bankomatu?",
      options: [
        "Ano, ale jen z bankomatu stejné banky",
        "Ano, z libovolného bankomatu, když zaplatím poplatek",
        "Ano, ale jen na pobočce",
        "Ne, musím si je poslat na běžný účet",
      ],
      answer: "Ne, musím si je poslat na běžný účet",
    },
    {
      question: "Můžu mít na spořícím účtu peníze v cizí měně?",
      options: [
        "Ano, ale jen v měnách stanovených bankou",
        "Ano, v jakékoli měně",
        "Ano, ale jen v eurech",
        "Ne, jen v korunách",
      ],
      answer: "Ano, ale jen v měnách stanovených bankou",
    },
    {
      question: "Jaké tvrzení o spoření v cizí měně je nepravdivé?",
      options: [
        "Úroky jsou zpravidla výrazně nižší",
        "Při platbě cizí měnou se může stát, že zaplatím více",
        "Nejčastější cizí měnou v ČR je dolar",
        "Spořící účty v cizí měně nabízí méně bank",
      ],
      answer: "Nejčastější cizí měnou v ČR je dolar",
    },
    {
      question: "Spořící účet se úročí zpravidla:",
      options: [
        "Jednoduchým úročením",
        "Spojitým úročením",
        "Složeným úročením",
        "Harmonickým úročením",
      ],
      answer: "Složeným úročením",
    },
    {
      question: "Co je to mínusový úrok na spořícím účtu?",
      options: [
        "Některé banky úročí peníze, které jsou fixované v odchozích platebních příkazech, tyto úroky pak strhnou další měsíc",
        "U vysokých částek banky dávají zpravidla mínusový úrok",
        "Banka si účtuje poplatek za vedení účtu",
        "Banka může účtovat záporný úrok, než si klienta prověří",
      ],
      answer:
        "Některé banky úročí peníze, které jsou fixované v odchozích platebních příkazech, tyto úroky pak strhnou další měsíc",
    },
    {
      question: "Od kolika let můžu mít spořící účet?",
      options: ["od 0 let", "od 15 let", "od 18 let", "od 21 let"],
      answer: "od 15 let",
    },
  ],
  buildigSaving: [
    {
      question:
        "Překoná zhodnocení na stavebním spoření zpravidla inflaci (bez státní podpory)?",
      options: ["Ano", "Ne"],
      answer: "Ne",
    },
    {
      question: "Jakou lze us stavebního spoření očekávat likviditu?",
      options: ["Okamžitou", "Vysokou", "Střední", "Nízkou"],
      answer: "Nízkou",
    },
    {
      question: "Jaký z následujících produktů je nejméně rizikový",
      options: [
        "Nemovitostní fond",
        "Stavební spoření",
        "Korporátní dluhopisový fond",
        "Akciový fond",
      ],
      answer: "Stavební spoření",
    },
    {
      question: "Do jaké částky je pojištěný vklad na stavebním spoření?",
      options: ["1 000 000 Kč", "50 000 €", "2 000 000 Kč", "100 000 €"],
      answer: "100 000 €",
    },
    {
      question: "Na co můžu použít peníze ze stavebního pojištění?",
      options: [
        "Pouze na koupi nemovitosti",
        "Na cokoliv, co si přeji",
        "Pouze na rekonstrukci nemovitosti",
        "Na koupi nebo rekonstrukci nemovitosti",
      ],
      answer: "Na cokoliv, co si přeji",
    },
    {
      question: "Jaké jsou výhody stavebního spoření?",
      options: [
        "Nízké riziko, státní podpora, možnost využití na cokoliv, daňová odečitatelnost",
        "Nízké riziko, státní podpora, možnost využití na cokoliv, vysoký výnos",
        "Daňová odečitatelnost, státní podpora, možnost využití na cokoliv, vysoký výnos",
        "Nízké riziko, vysoký výnos, daňová odečitatelnost",
      ],
      answer:
        "Nízké riziko, státní podpora, možnost využití na cokoliv, daňová odečitatelnost",
    },
    {
      question: "Je stavební spoření časově vázané?",
      options: ["Ano", "Ne, mohu si kdykoliv vybrat peníze bez postihu"],
      answer: "Ano",
    },
    {
      question: "Jaká je zpravidla vázací doba stavebního spoření?",
      options: ["4 roky", "6 let", "8 let", "10 let"],
      answer: "6 let",
    },
    {
      question:
        "Co se stane, když si peníze ze stavebního spoření vyberu dříve?",
      options: [
        "Ztratím státní podporu",
        "Musím zaplatit daň",
        "Musím zaplatit pokutu",
        "Nic, peníze si můžu kdykoliv vybrat",
      ],
      answer: "Ztratím státní podporu",
    },
    {
      question: "Jak vysoký je maximální státní příspěvek na stavební spoření?",
      options: ["1000 Kč", "2000 Kč", "3500 Kč", "4000 Kč"],
      answer: "1000 Kč",
    },
    {
      question: "Kolik % z vkládané částky můžu získat jako státní příspěvek?",
      options: ["5%", "10%", "15%", "20%"],
      answer: "5%",
    },
    {
      question:
        "Kolik je potřeba vložit ročně na stavební spoření, abych získal maximální státní příspěvek?",
      options: ["5 000 Kč", "10 000 Kč", "15 000 Kč", "20 000 Kč"],
      answer: "20 000 Kč",
    },
    {
      question:
        "Jaký můžu očekávat výnos ze stavebního spoření včetně státní podpory?",
      options: ["1-2%", "2-3%", "3-4%", "4-5%"],
      answer: "2-3%",
    },
    {
      question:
        "Kolik byl státní příspěvek na stavební spoření v době vzniku produktu?",
      options: ["500 Kč", "1000 Kč", "2000 Kč", "4500 Kč"],
      answer: "4500 Kč",
    },
    {
      question: "Jaký byl výnos ze stavebního spoření v době vzniku produktu?",
      options: ["0-2%", "2-4%", "4-6%", "6-8%"],
      answer: "4-6%",
    },
    {
      question: "Proč stát oficiálně primárně přispívá na stavební spoření?",
      options: [
        "Aby podpořil mladé lidi ve vlastnictví nemovitosti a spoření",
        "Aby podpořil stavební průmysl",
        "Aby podpořil banky",
        "Aby podpořil nemovitostní fondy",
      ],
      answer: "Aby podpořil mladé lidi ve vlastnictví nemovitosti",
    },
    {
      question: "Stát snižuje příspěvek na stavební spoření, protože:",
      options: [
        "Stát nemá neomezené zdroje, navíc zakládá nové produkty s podporou",
        "Státu se nevyplatí, aby mladí lidé měli vlastní nemovitost",
        "Státní podpora byla zneužívána",
        "Stát chce peníze využít spíše na podporu individuálních stavebních záměrů",
      ],
      answer:
        "Stát nemá neomezené zdroje, navíc zakládá nové produkty s podporou",
    },
    {
      question:
        "Jaký produkt nově vznikl k roku 2024 se státní podporou, do kterého se dá investovat?",
      options: [
        "Státní dluhopisy",
        "ETF - Exchange Traded Fund",
        "DIP - Dlouhodobý investiční produkt",
        "OIF - Občanský investiční fond",
      ],
      answer: "DIP - Dlouhodobý investiční produkt",
    },
    {
      question: "Má stavební spoření zpravidla minimální cílovou částku?",
      options: ["ano", "ne"],
      answer: "ano",
    },
    {
      question: "Musím na stavební spoření posílat peníze pravidelně?",
      options: [
        "Ano, každý měsíc musím poslat stejnou částku",
        "Ano, každý měsíc musím poslat alespoň minimální částku",
        "Ne, mohu posílat peníze kdykoliv uznám za vhodné",
        "Záleží na bance, některé mají povinnost měsíčního vkladu, jiné ne",
      ],
      answer:
        "Záleží na bance, některé mají povinnost měsíčního vkladu, jiné ne",
    },
    {
      question: "Má stavební spoření úroková pásma?",
      options: [
        "Ano, ale úrok klesá pomaleji, než u spořícího účtu",
        "Ne, úrok zůstává stejný",
        "Ano, jako spořící účet",
        "Ano, ale úrok klesá rychleji, než u spořícího účtu",
      ],
      answer: "Ne, úrok zůstává stejný",
    },
    {
      question: "Co je to druhá fáze stavebního spoření?",
      options: [
        "Fáze, kdy klient splácí státní podporu zpět státu",
        "Fáze, kdy klient využívá návazný produkt nemovitostního fondu",
        "Fáze úvěru, kdy si klient může půjčit peníze na nemovitost či rekonstrukci za výhodnějších podmínek",
        "Fáze žádosti o státní příspěvek na koupi či rekonstrukci nemovitosti",
      ],
      answer:
        "Fáze úvěru, kdy si klient může půjčit peníze na nemovitost či rekonstrukci za výhodnějších podmínek",
    },
    {
      question: "Musí klient využít druhou fázi stavebního spoření?",
      options: ["ano", "ne"],
      answer: "ne",
    },
    {
      question: "V úverové fázi stavebního spoření rozlišujeme:",
      options: [
        "Řádný a překlenovací úvěr",
        "Spotřebitelský a hypoteční úvěr",
        "Bankovní a nebankovní úvěr",
        "Rekonstrukční a stavební úvěr",
      ],
      answer: "Řádný a překlenovací úvěr",
    },
    {
      question: "Co je to řádný úvěr stavebního spoření?",
      options: [
        "Úvěr, který si klient bere až po ukončení stavebního spoření",
        "Úvěr, který nepřesahuje částku naspořenou klientem",
        "Úvěr, který je vázán minimálně na 10 let",
        "Úvěr, který má úrokovou sazbu nižší než 5%",
      ],
      answer: "Úvěr, který si klient bere až po ukončení stavebního spoření",
    },
    {
      question: "Co je to překlenovací úvěr stavebního spoření?",
      options: [
        "Úvěr, který si klient bere před ukončením stavebního spoření a doplácí jím zbývající částku",
        "Úvěr, který přesahuje částku naspořenou klientem",
        "Úvěr, který je vázán na méně než 10 let",
        "Úvěr, který má úrokovou sazbu vyšší než 5%",
      ],
      answer:
        "Úvěr, který si klient bere před ukončením stavebního spoření a doplácí jím zbývající částku",
    },
    {
      question:
        "Má vyšší úrokovou sazbu zpravidla řádný nebo překlenovací úvěr?",
      options: ["řádný úvěr", "překlenovací úvěr"],
      answer: "překlenovací úvěr",
    },
    {
      question: "Kdy u nás vznikly první stavební spořitelny?",
      options: [
        "Za první republiky (1928)",
        "Za protektorátu (1942)",
        "Po roce 1948",
        "Po sametové revoluci (1990)",
      ],
      answer: "Za první republiky (1928)",
    },
    {
      question: "Proč stavební spoření nefungovalo během socialistické éry?",
      options: [
        "Individuální spoření na nemovitost nebylo v socialistickém systému podporováno",
        "Stát neměl dostatek peněz na podporu stavebního spoření",
        "Bydlení bylo poskytováno státem",
        "Stavební spoření bylo zneužíváno k úniku peněz do zahraničí",
      ],
      answer:
        "Individuální spoření na nemovitost nebylo v socialistickém systému podporováno",
    },
    {
      question: "Kolik je sjednaných stavebních spoření v ČR?",
      options: [
        "Asi 1 milion",
        "Asi 2 miliony",
        "Asi 3 miliony",
        "Asi 4 miliony",
      ],
      answer: "Asi 3 miliony",
    },
    {
      question: "Jak jsou zdaněny výnosy ze stavebního spoření?",
      options: [
        "Výnosy jsou osvobozeny od daně",
        "5% daň z výnosů",
        "10% daň z výnosů",
        "15% daň z výnosů",
      ],
      answer: "15% daň z výnosů",
    },
    {
      question: "Můžu sjednat stavební spoření i pro dítě?",
      options: [
        "Ano, zákonný zástupce může sjednat stavební spoření od narození dítěte",
        "Ano, zákonný zástupce může sjednat stavební spoření od 15 let dítěte",
        "Ano, dítě si může sjednat stavební spoření od 15 let",
        "Ne, stavební spoření je určeno jen pro plnoleté",
      ],
      answer:
        "Ano, zákonný zástupce může sjednat stavební spoření od narození dítěte",
    },
    {
      question: "Dostávám státní podporu i v úrokové fázi stavebního spoření?",
      options: [
        "Ano, ale sníženou na polovinu",
        "Ano, v plné výši",
        "Ne",
        "Záleží na bance",
      ],
      answer: "Ne",
    },
    {
      question:
        "Pokud ukončím stavební spoření předčasně z důvodu přespoření, ztrácím státní podporu?",
      options: ["ano", "ne"],
      answer: "ne",
    },
    {
      question: "Kolik stavebních spořitelen působí na českém trhu?",
      options: ["5", "10", "15", "20"],
      answer: "5",
    },
    {
      question: "Která z následujících institucí není stavební spořitelnou?",
      options: [
        "ČSOB Stavební spořitelna, a.s.",
        "Raiffeisen stavební spořitelna a.s.",
        "Unicredit stavební spořitelna, a.s.",
        "Modrá pyramida stavební spořitelna, a.s.",
      ],
      answer: "Unicredit stavební spořitelna, a.s.",
    },
  ],
  termDeposit: [
    {
      question: "Termínovaný vklad považujeme za: ",
      options: ["investování", "spoření", "spekulaci", "likvidní rezervu"],
      answer: "spoření",
    },
    {
      question: "V jaké instituci můžu sjednat termínovaný vklad?",
      options: [
        "Pouze v bance",
        "V bance nebo v záložně",
        "V bance, záložně nebo u staební spořitelny",
        "V záložně nebo u stavební spořitelny",
      ],
      answer: "V bance nebo v záložně",
    },
    {
      question: "Jaký je rozdíl mezi termínovaným vkladem u banky a u záložny?",
      options: [
        "Vklad u záložny není pojištěn, u banky ano",
        "Vklad u banky a záložny je totožný",
        "Vklad u záložny je možné kdykoliv vypovědět bez postihu",
        "Vklad u záložny je podmíněn členským vkladem",
      ],
      answer: "Vklad u záložny je podmíněn členským vkladem",
    },
    {
      question: "Do jaké částky je termínovaný vklad pojištěn?",
      options: [
        "Termínovaný vklad se nepojišťuje",
        "Do 10 000 €",
        "Do 50 000 €",
        "Do 100 000 €",
      ],
      answer: "Do 100 000 €",
    },
    {
      question: "Pokud sjednávám vklad u záložny, jak je vysoký členský vklad?",
      options: ["1%", "2%", "5%", "10%"],
      answer: "10%",
    },
    {
      question: "Je členský vklad v družstevní záložně pojištěn?",
      options: [
        "Členský vklad není pojištěný",
        "Do 10 000 €",
        "Do 50 000 €",
        "Do 100 000 €",
      ],
      answer: "Členský vklad není pojištěný",
    },
    {
      question: "Kolik je v ČR k roku 2024 družstevních záložen?",
      options: ["5", "6", "7", "8"],
      answer: "6",
    },
    {
      question: "Kolik % Čechů má termínovaný vklad?",
      options: ["Méně než 20%", "20-40%", "40-60%", "Více než 60%"],
      answer: "Méně než 20%",
    },
    {
      question:
        "U delších termínovaných vkladů (za neměnných ekonomických podmínek) je úrok zpravidla:",
      options: ["Vyšší, než u kratších vkladů", "Nižší, než u kratších vkladů"],
      answer: "Vyšší, než u kratších vkladů",
    },
    {
      question:
        "V případě, že je aktuálně vysoká inflace, ale očekává se zlepšení situace:",
      options: [
        "Delší vklady budou mít vyšší úrok, než kratší",
        "Kratší vklady budou mít vyšší úrok, než delší",
        "Úrok je řízen jinými faktory, než je inflace",
        "Vklady budou mít stejný úrok",
      ],
      answer: "Kratší vklady budou mít vyšší úrok, než delší",
    },
    {
      question: "Má termínovaný vklad úroková pásma?",
      options: [
        "Ano, úrok ale klesá pomaleji, než u spořícího účtu",
        "Ano, úrok se snižuje na 50% po dosažení 1 000 000 Kč",
        "Ano, úrok klesá jako na spořícím účtu",
        "Ne",
      ],
      answer: "Ne",
    },
    {
      question: "Můžu sjednat termínovaný vklad i pro dítě?",
      options: [
        "Ano, zákonný zástupce může sjednat termínovaný vklad od narození dítěte",
        "Ano, zákonný zástupce může sjednat termínovaný vklad od 15 let dítěte",
        "Ano, dítě si může sjednat termínovaný vklad od 15 let",
        "Ne, termínovaný vklad je určeno jen pro plnoleté",
      ],
      answer:
        "Ano, zákonný zástupce může sjednat termínovaný vklad od 15 let dítěte",
    },
    {
      question: "Co rozumíme krátkodobým termínovaným vkladem?",
      options: [
        "Vkladem, který nemá delší vázací dobu než 1 měsíc",
        "Vkladem, který nemá delší vázací dobu než 1 rok",
        "Vkladem, který nemá delší vázací dobu než 5 let",
        "Vkladem, který nemá delší vázací dobu než 10 let",
      ],
      answer: "Vkladem, který nemá delší vázací dobu než 1 rok",
    },
    {
      question: "Střednědobý termínovaný vklad má zpravidla vázací dobu:",
      options: ["1-2 roky", "1-5 let", "2-10 let", "5-20 let"],
      answer: "1-5 let",
    },
    {
      question: "Dlouhodobý termínovaný vklad má zpravidla vázací dobu:",
      options: [
        "5 a více let",
        "10 a více let",
        "20 a více let",
        "50 a více let",
      ],
      answer: "5 a více let",
    },
    {
      question: "Jak dělíme termínované vklady podle způsobu ukončení",
      options: [
        "Na jednorázové a revolvingové",
        "Na vázané a výběrové",
        "Na smluvní a dohodové",
        "Na pevné a flexibilní",
      ],
      answer: "Na jednorázové a revolvingové",
    },
    {
      question: "Jaký je rozdíl mezi jednorázovým a revolvingovým vkladem?",
      options: [
        "Jednorázový vklad může mít klient u jedné banky pouze jednou, revolvingových může klient uzavřít několik paralelně",
        "Jednorázový vklad má zpravidla vyšší úrok, revolvingový má nižší",
        "Jednorázový vklad se vybere po skončení vázací lhůty, revolvingový se po stanovené lhůtě obnoví, pokud ho klient nevybere",
        "Jednorázový vklad má zpravidla vyšší likviditu, revolvingový nižší",
      ],
      answer:
        "Jednorázový vklad se vybere po skončení vázací lhůty, revolvingový se po stanovené lhůtě obnoví, pokud ho klient nevybere",
    },
    {
      question:
        "Jaký je rozdíl mezi fixním a variabilním úročením termínovaného vkladu?",
      options: [
        "Fixní úrok je základní úrok, variabilní je bonusový",
        "Fixní úrok je základní úrok, variabilní je úrok z výnosu",
        "Fixní úrok je likvidní, variabilní je spořící",
        "Fixní úrok je po celou dobu neměnný, variabilní se mění s aktuální situací na trhu",
      ],
      answer:
        "Fixní úrok je po celou dobu neměnný, variabilní se mění s aktuální situací na trhu",
    },
    {
      question: "V bankách se termínovaný vklad zpravidla sjednává:",
      options: ["S fixním úrokem", "S variabilním úrokem"],
      answer: "S fixním úrokem",
    },
    {
      question:
        "Můžu u termínovaný vklad vybrat peníze před termínem bez postihu?",
      options: ["Ano", "Ne"],
      answer: "Ne",
    },
    {
      question: "Co se stane, pokud si klient vybere peníze před termínem?",
      options: [
        "Klient si peníze předčasně vybrat nemůže za žádných okolností",
        "Klient zaplatí odstupný poplatek, který není zpravidla vysoký",
        "Klient zpravidla přijde o úrok, zaplatí penalizaci, případně kombinaci obou",
        "Z termínovaného vkladu si klient může kdykoliv vybrat peníze bez postihu",
      ],
      answer:
        "Klient zpravidla přijde o úrok, zaplatí penalizaci, případně kombinaci obou",
    },
    {
      question:
        "Může klient do termínovaného vkladu přidávat peníze během jeho trvání?",
      options: [
        "Ano, bez jakýchkoliv omezení",
        "Ano, ale pouze do výše původního vkladu",
        "Ano, ale s poplatkem 2% z vkládané částky",
        "Ne, termínovaný vklad je neměnný",
      ],
      answer: "Ne, termínovaný vklad je neměnný",
    },
    {
      question: "Jakému zdanění podléhají výnosy z termínovaného vkladu?",
      options: [
        "15% daň z výnosů",
        "5% daň z výnosů",
        "10% daň z výnosů",
        "Výnosy jsou osvobozeny od daně",
      ],
      answer: "15% daň z výnosů",
    },
    {
      question: "Je úroková míra zpravidla vyšší než u spořících účtů?",
      options: ["Ano", "Ne"],
      answer: "Ano",
    },
    {
      question:
        "Za jaké situace může mít termínovaný vklad nižší úrok, než spořící účet?",
      options: [
        "Pokud je nízká inflace, u které se očekává zhoršení",
        "Pokud je vysoká inflace, u které se očekává zhoršení",
        "Pokud je inflace stabilní",
        "Pokud je vysoká inflace, u které se očekává zlepšení",
      ],
      answer: "Pokud je vysoká inflace, u které se očekává zlepšení",
    },
    {
      question: "Jaká je likvidita termínovaného vkladu?",
      options: ["Okamžitá", "Vysoká", "Střední", "Nízká"],
      answer: "Nízká",
    },
    {
      question:
        "Pokud je ve smlouvě u termínovaného vkladu uvedeno, že je úročená částka připisovaná na běžný účet, jedná se o",
      options: [
        "Jednoduché úročení",
        "Harmonické úročení",
        "Složené úročení",
        "Kombinované úročení",
      ],
      answer: "Jednoduché úročení",
    },
    {
      question: "Mají termínované vklady zpravidla minimální vklad?",
      options: ["Ano", "Ne"],
      answer: "Ano",
    },
    {
      question: "Mají termínované vklady zpravidla maximální vklad?",
      options: ["Ano", "Ne"],
      answer: "Ne",
    },
    {
      question: "Mohu mít více termínovaných vkladů u jedné banky?",
      options: ["Ano", "Ne"],
      answer: "Ano",
    },
    {
      question:
        "Chci spořit 3 000 000 Kč na dlouhodobější období, je obvykle lepší spořící účet nebo termínovaný vklad?",
      options: [
        "Spořící účet, protože se k penězům snáze dostanu",
        "Spořící účet, protože je vklad na něm pojištěn do vyšší částky",
        "Termínovaný vklad, protože má vyšší úrok",
        "Termínovaný vklad, protože má nižší riziko",
      ],
      answer: "Termínovaný vklad, protože má zpravidla vyšší úrok",
    },
    {
      question: "Mohu zpravidla spořit na termínovaný vklad i v cizí měně?",
      options: [
        "Ano, v jakékoliv měně",
        "Ano, ale většinou jen v Eurech a Dolarech",
        "Ano, ale měna se automaticky převádí na Kč",
        "Ne, termínovaný vklad je vždy v Kč",
      ],
      answer: "Ano, ale většinou jen v Eurech a Dolarech",
    },
    {
      question: "Jak je vysoké riziko termínovaného vkladu?",
      options: ["Velmi vysoké", "Vysoké", "Střední", "Nízké"],
      answer: "Nízké",
    },
  ],
  pensionSaving: [
    {
      question: "Kde můžu sjednat doplňkové penzijní spoření v ČR?",
      options: [
        "Pouze u národní banky, jedná se o státní produkt",
        "U komerčních bank",
        "U pojišťoven",
        "U nebankovních finančních institucí",
      ],
      answer: "U komerčních bank",
    },
    {
      question: "Co zpravidla zvyšuje atraktivitu penzijního spoření?",
      options: [
        "Možnost výběru peněz kdykoliv",
        "Vysoké procentuální zhodnocení",
        "Státní příspěvek",
        "Možnost spoření v cizí měně",
      ],
      answer: "Státní příspěvek",
    },
    {
      question: "Patří penzijní spoření mezi investování nebo spoření?",
      options: [
        "Spoření",
        "Investování",
        "Spekulace",
        "Kombinace spoření a investování",
      ],
      answer: "Kombinace spoření a investování",
    },
    {
      question: "Mohu u penzijního spoření dosáhnout záporného zhodnocení?",
      options: ["Ano", "Ne"],
      answer: "Ano",
    },
    {
      question: "Jaká je likvidita penzijního spoření?",
      options: ["Okamžitá", "Vysoká", "Střední", "Nízka"],
      answer: "Nízká",
    },
    {
      question:
        "Kolik je k roku 2024 sjednaných doplňkových penzijních spoření?",
      options: ["900 000", "1 700 000", "2 500 000", "3 300 000"],
      answer: "1 700 000",
    },
    {
      question: "V jakém roce vzniklo doplňkové penzijní spoření v ČR?",
      options: ["1948", "1989", "1993", "2013"],
      answer: "2013",
    },
    {
      question: "Jaký produkt existoval před doplňkovým penzijním spořením?",
      options: [
        "Žádný, doplňkové penzijní spoření nemá předchůdce",
        "Penzijní připojištění",
        "Dlouhodobý investiční produkt",
        "Starobní důchod",
      ],
      answer: "Penzijní připojištění",
    },
    {
      question: "Kolik starých penzijních připojištění ještě stále běží?",
      options: ["0", "1 000 000", "2 500 000", "4 000 000"],
      answer: "2 500 000",
    },
    {
      question: "Je doplňkové penzijní spoření podporováno státem?",
      options: ["Ano", "Ne"],
      answer: "Ano",
    },
    {
      question: "Jaká je nejnižší částka, kterou mohu spořit?",
      options: [
        "Žádná, mohu spořit kolik chci",
        "100 Kč",
        "500 Kč",
        "1 000 Kč",
      ],
      answer: "100 Kč",
    },
    {
      question: "Od jaké částky stát přispívá na penzijní spoření?",
      options: ["100 Kč", "500 Kč", "1 000 Kč", "2 000 Kč"],
      answer: "500 Kč",
    },
    {
      question: "Jak vysoký je státní příspěvek na penzijní spoření?",
      options: [
        "5 % z vkládané částky",
        "10 % z vkládané částky",
        "20 % z vkládané částky",
        "30 % z vkládané částky",
      ],
      answer: "20 % z vkládané částky",
    },
    {
      question: "Do jaké maximální částky stát přispívá na penzijní spoření?",
      options: ["1 700 Kč", "5 000 Kč", "10 000 Kč", "20 000 Kč"],
      answer: "1 700 Kč",
    },
    {
      question:
        "Jak vysoký je nejvyšší možný státní příspěvek na penzijní spoření?",
      options: ["340 Kč", "500 Kč", "1 000 Kč", "2 000 Kč"],
      answer: "340 Kč",
    },
    {
      question: "Do jaké částky je doplňkové penzijní spoření pojištěno?",
      options: [
        "Doplňkové penzijní spoření se nepojišťuje",
        "50 000 €",
        "100 000 €",
        "200 000 €",
      ],
      answer: "Doplňkové penzijní spoření se nepojišťuje",
    },
    {
      question: "Můžu sjednat doplňkové penzijní spoření i pro dítě?",
      options: [
        "Ano, od narození",
        "Ano, od patnácti let",
        "Ano, dítě si samo může sjednat spoření od patnácti let",
        "Ne, stavební spoření je dostupné pouze pro dospělé",
      ],
      answer: "Ano, od narození",
    },
    {
      question:
        "Mohu peníze z doplňkového penzijního spoření kdykoliv vybrat bez postihu?",
      options: ["Ano", "Ne"],
      answer: "Ne",
    },
    {
      question:
        "Co se stane, pokud si klient vybere peníze dříve, než po 2 letech?",
      options: [
        "Klient musí zdanit zisk daní z příjmu 15 %",
        "Klient zaplatí odstupný poplatek ve výši 10 % z vybrané částky a přijde o státní příspěvek",
        "Peníze propadnou ve prospěch penzijní společnosti",
        "Klient musí zaplatit fixní pokutu 10 000 Kč",
      ],
      answer: "Peníze propadnou ve prospěch penzijní společnosti",
    },
    {
      question:
        "Jaký je požadavek, aby klient mohl vybrat peníze z penzijního spoření bez postihu?",
      options: [
        "Klient musí mít naspořeno alespoň 100 000 Kč",
        "Klient musí dovršit 60 let a spořit minimálně 10 let",
        "Klient musí spořit minimálně 10 let",
        "Klient musí dovršit 60 let",
      ],
      answer: "Klient musí dovršit 60 let a spořit minimálně 10 let",
    },
    {
      question:
        "Co se stane, pokud klient vybere peníze z doplňkového penzijního spoření předčasně ale po 2 letech?",
      options: [
        "Přijde o státní příspěvek a zaplatí daň z příjmu 15 %",
        "Přijde o státní příspěvek a zaplatí daň z příjmu na výnosech a posledních 10 let příspěvků od zaměstnavatele",
        "Přijde o státní příspěvek, peníze od zaměstnavatele i připsané úroky",
        "Přijde pouze o státní příspěvek",
      ],
      answer:
        "Přijde o státní příspěvek a zaplatí daň z příjmu na výnosech a posledních 10 let příspěvků od zaměstnavatele",
    },
    {
      question:
        "Pokud se klient stane invalidním třetího stupně, může penzijní spoření vybrat?",
      options: [
        "Ne, klient musí počkat do svých 60 let",
        "Ano, pokud spoří alespoň 3 roky, bude mu společnost vyplácet postupně peníze",
        "Ano, klient může vybrat celou částku",
        "Ano, klient může vybrat částku za podmínky, že zdaní zisk daní z příjmu 15%",
      ],
      answer:
        "Ano, pokud spoří alespoň 3 roky, bude mu společnost vyplácet postupně peníze",
    },
    {
      question: "Pokud klient dovrší 60 let a spoří alespoň 10 let: ",
      options: [
        "Dostane automaticky částku na účet",
        "Může si nechat částku vyplácet ve formě renty nebo ji vybrat celou",
        "Musí počkat dalších 5 let, než může částku vybrat",
        "Musí zaplatit výstupní poplatek za zprávu fondu, která je rovná 1 % z naspořené částky",
      ],
      answer:
        "Může si nechat částku vyplácet ve formě renty nebo ji vybrat celou",
    },
    {
      question:
        "Pokud po splnění všech podmínek klient vybere celou částku najednou: ",
      options: [
        "Částka je oproštěna jakýchkoliv dalších daní a srážek",
        "Klient musí zdanit výnos daní z příjmu 15 %, příspěvky od zaměstnavatele jsou od daně osvobozeny",
        "Klient musí zdanit příspěvky zaměstnavatele daní z příjmu 15 %",
        "Klient musí zdanit výnos a příspěvky zaměstnavatele daní z příjmu 15 %",
      ],
      answer:
        "Klient musí zdanit výnos a příspěvky zaměstnavatele daní z příjmu 15 %",
    },
    {
      question: "Pokud klient zvolí rentu, která je kratší než 10 let: ",
      options: [
        "Klient nemusí nic platit, stát ani penzijní společnost mu nic vzít nesmí",
        "Klient musí zdanit výnos daní z příjmu 15 %, příspěvky od zaměstnavatele jsou od daně osvobozeny",
        "Klient musí zdanit příspěvky zaměstnavatele daní z příjmu 15 %",
        "Klient musí zdanit výnos a příspěvky zaměstnavatele daní z příjmu 15 %",
      ],
      answer:
        "Klient musí zdanit výnos daní z příjmu 15 %, příspěvky od zaměstnavatele jsou od daně osvobozeny",
    },
    {
      question: "Pokud klient zvolí rentu, která je delší než 10 let: ",
      options: [
        "Klient nemusí nic platit, stát ani penzijní společnost mu nic vzít nesmí",
        "Klient musí zdanit výnos daní z příjmu 15 %, příspěvky od zaměstnavatele jsou od daně osvobozeny",
        "Klient musí zdanit příspěvky zaměstnavatele daní z příjmu 15 %",
        "Klient musí zdanit výnos a příspěvky zaměstnavatele daní z příjmu 15 %",
      ],
      answer:
        "Klient nemusí nic platit, stát ani penzijní společnost mu nic vzít nesmí",
    },
    {
      question:
        "Pokud se klient rozhodne si doplňkové penzijní spoření sjednat, na jakou rezervu je vhodné ho využít?",
      options: [
        "Na krátkodobou",
        "Na střednědobou",
        "Na střednědobou nebo dlouhodobou",
        "Na dlouhodobou",
      ],
      answer: "Na dlouhodobou",
    },
    {
      question:
        "Může si klient zvolit, jak rizikově bude penzijní společnost vložené peníze investovat?",
      options: [
        "Ano, banky zpravidla nabízejí více možností",
        "Ano, každá banka má jednu vlastní rizikovost",
        "Ne, penzijní společnost má povinnost investovat peníze do nejméně rizikových aktiv",
        "Ne, penzijní společnost předává peníze do státního fondu",
      ],
      answer: "Ano, banky zpravidla nabízejí více možností",
    },
    {
      question: "Je doplňkové penzijní spoření daňově odpočitatelné?",
      options: ["Ano", "Ne"],
      answer: "Ano",
    },
    {
      question:
        "Jak funguje daňové odpočtení u doplňkového penzijního spoření?",
      options: [
        "Klient může od základu daně odepsat až 1 000 Kč měsíčně, které přesahují 1 700 Kč",
        "Klient může od základu daně odepsat až 2 000 Kč měsíčně, které přesahují 1 700 Kč",
        "Klient může od základu daně odepsat až 4 000 Kč měsíčně, které přesahují 1 700 Kč",
        "Klient může od základu daně odepsat až 8 000 Kč měsíčně, které přesahují 1 700 Kč",
      ],
      answer:
        "Klient může od základu daně odepsat až 4 000 Kč měsíčně, které přesahují 1 700 Kč",
    },
    {
      question:
        "Kolik klient ročně uspoří, pokud využije maximálního státního příspěvku a maximální daňové odečitatelnosti?",
      options: [
        "4 080 Kč na příspěvcích a 7 200 Kč na daních",
        "4 080 Kč na příspěvcích a 48 000 Kč na daních",
        "20 400 Kč na příspěvcích a 7 200 Kč na daních",
        "20 400 Kč na příspěvcích a 48 000 Kč na daních",
      ],
      answer: "4 080 Kč na příspěvcích a 7 200 Kč na daních",
    },
    {
      question: "V porovnání se spořícím účtem má doplňkové penzijní spoření: ",
      options: [
        "Extrémně nízkou likviditu, vyšší výnos a vyšší rizikovost",
        "Extrémně nízkou, nižší výnos a nižší rizikovost",
        "Extrémně vysokou likviditu, vyšší výnos a nižší rizikovost",
        "Extrémně vysokou likviditu, nižší výnos a vyšší rizikovost",
      ],
      answer: "Extrémně nízkou likviditu, vyšší výnos a vyšší rizikovost",
    },
    {
      question:
        "V porovnání se stavebním spořením má doplňkové penzijní spoření: ",
      options: [
        "Nižší likviditu, nižší výnos a nižší rizikovost",
        "Nižší likviditu, větší výnos a vyšší rizikovost",
        "Vyšší likviditu, nižší výnos a nižší rizikovost",
        "Vyšší likviditu, větší výnos a vyšší rizikovost",
      ],
      answer: "Nižší likviditu, větší výnos a vyšší rizikovost",
    },
    {
      question: "Může penzijní spoření překonat inflaci?",
      options: [
        "Ano, penzijní spoření zpravidla překonává inflaci",
        "Ano, pokud je zvolen rizikovější fond",
        "Ne, penzijní spoření přesahuje inflaci jen velmi výjimečně",
        "Ne, penzijní spoření je nižší než inflace",
      ],
      answer: "Ano, pokud je zvolen rizikovější fond",
    },
    {
      question: "Prostředky na doplňkovém penzijním spoření jsou úročeny: ",
      options: [
        "Jednoduchým úročením",
        "Složeným úročením",
        "Harmonickým úročením",
        "Kvadratickým úročením",
      ],
      answer: "Složeným úročením",
    },
    {
      question:
        "Marek má 25 let a sjednal si doplňkové penzijní spoření. Vkládá 1 700 Kč měsíčně a od zaměstnavatele dostává 500 Kč měsíčně. Zvolil variantu s očekávaným úrokem 4 % ročně. Kolik bude mít naspořeno v šedesáti letech?",
      options: [
        "Asi 1 400 000 Kč",
        "Asi 2 300 000 Kč",
        "Asi 3 200 000 Kč",
        "Asi 4 300 000 Kč",
      ],
      answer: "Asi 2 300 000 Kč",
    },
  ],
  investmentFunds: [
    {
      question: "Jaký je rozdíl mezi spořením a investováním?",
      options: [
        "Spoření a investování mají stejný význam",
        "Investování je jen pro bohaté, spoření pro běžné lidi",
        "Spoření je více ovlivněno krizemi finančního trhu",
        "Při investic vystavuji peníze riziku ztráty, u spoření ne",
      ],
      answer: "Při investic vystavuji peníze riziku ztráty, u spoření ne",
    },
    {
      question: "Fondy kolektivního investování rozumíme: ",
      options: [
        "Neregulované majetkové fondy",
        "Přísně regulované a dozorované fondy určené pro širokou veřejnost",
        "Fondy poskytující kapitálové životní pojištění",
        "Neregulované strukturované fondy",
      ],
      answer:
        "Přísně regulované a dozorované fondy určené pro širokou veřejnost",
    },
    {
      question: "V čem spočívá podstata fondů kolektivního investování?",
      options: [
        "Investoři (členové fondu) se schází a radí se o tom, kam investovat",
        "Sdružení peněz od investorů a investování je do různých aktiv",
        "Cílem fondu je shromažďovat peníze od investorů a půjčovat je jiným investorům",
        "Cílem fondu je shromažďovat peníze od investorů a zajišťovat jim úrok",
      ],
      answer:
        "Fond kolektivního investování sdružuje peníze od investorů a investuje je do různých aktiv",
    },
    {
      question: "Jaké z následujících tvrzení o vstupních poplatcích neplatí?",
      options: [
        "Je hrazen investorem nakupujícím podílové listy či akcie fondu.",
        "Nesmí být účtován distributorem fondu vyšší než připouští statut fondu.",
        "Může být účtován při nákupu podílových listů či akcií fondu v různé výši v závislosti na ceníku různých distributorů",
        "Nesmí být účtován v případě, že fond aplikuje též výstupní poplatek",
      ],
      answer:
        "Nesmí být účtován v případě, že fond aplikuje též výstupní poplatek",
    },
    {
      question: "Můžu při nákupu fondu ztratit víc, než jsem investoval?",
      options: [
        "Ano, pokud se hodnota fondu sníží",
        "Ano, ale do maximální výše variabilního poplatku (25% z investice)",
        "Ne, vždy získám zpět alespoň to, co jsem investoval",
        "Ne, pokud fond nezkrachuje",
      ],
      answer: "Ano, pokud se hodnota fondu sníží",
    },
    {
      question: "Co je to aktivní správa fondu?",
      options: [
        "Forma správy fondu, kdy správce musí informovat investory o každé změně v portfoliu",
        "Forma správy fondu, kdy správce vyhledává, nakupuje a prodává svěřená aktiva",
        "Forma správy fondu, kdy je fond spravován automaticky",
        "Forma správy fondu, kdy správce nakupuje pouze volatilní aktiva",
      ],
      answer:
        "Forma správy fondu, kdy správce vyhledává, nakupuje a prodává svěřená aktiva",
    },
    {
      question: "Co je to pasivní správa fondu?",
      options: [
        "Forma správy fondu, kdy správce nakupuje pouze stabilní aktiva",
        "Forma správy fondu, kdy je fond spravován na základě algoritmu",
        "Forma správy fondu, kdy správce nemusí informovat investory o změnách v portfoliu",
        "Forma správy fondu, kdy správce zpravidla nemění složení portfolia",
      ],
      answer:
        "Forma správy fondu, kdy správce zpravidla nemění složení portfolia",
    },
    {
      question:
        "Bude mít zpravidla vyšší poplatky aktivně či pasivně spravovaný fond?",
      options: [
        "Pasivně spravovaný fond",
        "Aktivně spravovaný fond",
        "Výše poplatků nezávisí na způsobu správy",
        "Oba typy fondů mají stejné poplatky",
      ],
      answer: "Aktivně spravovaný fond",
    },
    {
      question: "Co je to volatilita investičního nástroje?",
      options: [
        "Volatilita popisuje, jak vysoký je potenciální maximální zisk",
        "Volatilita popisuje, jak vysoká je pravděpodobnost ztráty investice",
        "Volatilita popisuje, jak moc velké jsou výkyvy hodnoty investičního nástroje",
        "Volatilita popisuje souhrn likvidity a zisku investičního nástroje",
      ],
      answer:
        "Volatilita popisuje, jak moc velké jsou výkyvy hodnoty investičního nástroje",
    },
    {
      question: "Ukazatel SRRI vyjadřuje:",
      options: [
        "Na stupnici 1 - 10 likviditu fondu vycházející z historické volatility fondu",
        "Na stupnici 1 - 10 rizikovost fondu vycházející z jeho historické volatility.",
        "Na stupnici 1 - 7 rizikovost fondu vycházející z jeho zařazení do definovaných sedmi tříd aktiv.",
        "Na stupnici 1 - 7 rizikovost fondu vycházející z jeho historické volatility.",
      ],
      answer:
        "Na stupnici 1 - 7 rizikovost fondu vycházející z jeho historické volatility.",
    },
    {
      question:
        "Jaký bude vztah mezi ukazatelem SRRI běžného fondu peněžního trhu a běžného akciového fondu?",
      options: [
        "Vzhledem k nižší volatilitě nástrojů peněžního trhu než je volatilita akcií, bude SRRI fondu peněžního trhu nižší",
        "Vzhledem k nižší volatilitě nástrojů peněžního trhu než je volatilita akcií, bude SRRI fondu peněžního trhu vyšší",
        "Vzhledem k nižší likviditě nástrojů peněžního trhu než je likvidita akcií, bude SRRI fondu peněžního trhu vyšší",
        "Vzhledem k vyšší likviditě nástrojů peněžního trhu než je likvidita akcií, bude SRRI fondu peněžního trhu nižší",
      ],
      answer:
        "Vzhledem k nižší volatilitě nástrojů peněžního trhu než je volatilita akcií, bude SRRI fondu peněžního trhu nižší",
    },
    {
      question: "Jsou fondy k překonávání inflace vhodnější než spořící účet?",
      options: ["ano", "ne"],
      answer: "ano",
    },
    {
      question: "Co je to zpětný odkup podílových listů?",
      options: [
        "Možnost komukoliv prodat podílové listy z fondu",
        "Možnost správy fondu kdykoliv odkoupit podílové listy od investora",
        "Garance možnosti kdykoliv prodat podílové listy",
        "Garance odkupu podílových listů za stejnou cenu, za jakou byly zakoupeny",
      ],
      answer: "Garance možnosti kdykoliv prodat podílové listy",
    },
    {
      question: "Fondy kolektivního investování mají zpravidla",
      options: [
        "Okamžitou likviditu",
        "Vysokou likviditu",
        "Střední likviditu",
        "Nízkou likviditu",
      ],
      answer: "Střední likviditu",
    },
    {
      question: "Jaký z následujících nástrojů je nejlikvidnější?",
      options: [
        "Stavební spoření",
        "Termínovaný vklad",
        "Fond kolektivního investování",
        "Doplňkové penzijní spoření",
      ],
      answer: "Fond kolektivního investování",
    },
    {
      question:
        "Jaký z následujících nástrojů je nejvhodnější k překonání inflace?",
      options: [
        "Termínovaný vklad",
        "Spořící účet",
        "Fond kolektivního investování",
        "Stavební spoření",
      ],
      answer: "Fond kolektivního investování",
    },
    {
      question:
        "Na tvorbu jaké finanční rezervy je vhodný fond kolektivního investování?",
      options: [
        "Krátkodobé",
        "Střednědobé",
        "Dlouhodobé",
        "Na okamžité utrácení",
      ],
      answer: "Dlouhodobé",
    },
    {
      question:
        "Proč se doporučuje při investice do podílového fondu volit delší horizont?",
      options: [
        "Fond počítá s tím, že celková ekonomika roste, krátkodobě ale může klesat",
        "Při delší investici klesají poplatky",
        "Delší horizont znamená vyšší riziko, ale i vyšší potenciální výnos",
        "Při delší investici dostává investor pasivní výnos z příchodu nových investorů",
      ],
      answer:
        "Fond počítá s tím, že celková ekonomika roste, krátkodobě ale může klesat",
    },
    {
      question: "Do jaké částky je investice do podílového fondu pojištěna?",
      options: [
        "Investice do podílového fondu není pojištěna",
        "Do 50 000 €",
        "Do 100 000 €",
        "Do 200 000 €",
      ],
      answer: "Investice do podílového fondu není pojištěna",
    },
    {
      question: "Co je to dluhopis?",
      options: [
        "Forma akcie, která zajišťuje podíl na zisku společnosti",
        "Krátkodobý finanční nástroj s vysokým rizikem",
        "Půjčka, kterou si investor může vzít od emitenta",
        "Cenný papír, který představuje půjčku poskytnutou investorem emitentovi",
      ],
      answer:
        "Cenný papír, který představuje půjčku poskytnutou investorem emitentovi",
    },
    {
      question:
        "Emitentem dluhopisu může být za předpokladu splnění všech podmínek",
      options: [
        "Jakákoliv právnická nebo fyzická osoba",
        "Pouze obchodní společnost",
        "Pouze územní samosprávný celek (kraj, obec, stát)",
        "Pouze banka či jiná finanční instituce",
      ],
      answer: "Jakákoliv právnická nebo fyzická osoba",
    },
    {
      question: "Co je to akcie?",
      options: [
        "Cenný papír, který představuje půjčku poskytnutou investorem emitentovi",
        "Bezrizikový finanční nástroj s proměnlivým úrokem",
        "Cenný papír, který představuje podíl na majetku a zisku emitenta",
        "Forma korporátního dluhopisu s pevnou úrokovou sazbou",
      ],
      answer:
        "Cenný papír, který představuje podíl na majetku a zisku emitenta",
    },
    {
      question: "Co je to komodita?",
      options: [
        "Měnový poukázka opravňující ke směně za jinou měnu v předem daném kurzu",
        "Fyzické suroviny nebo produkty, které jsou obchodovány na trzích, jako jsou zlato, ropa, pšenice",
        "Podíl na hmotném majetku společnosti (nemovitosti, výrobní prostředky...)",
        "Podíl na nehmotném majetku společnosti (značka, know-how...)",
      ],
      answer:
        "Fyzické suroviny nebo produkty, které jsou obchodovány na trzích, jako jsou zlato, ropa, pšenice",
    },
    {
      question: "Fond investující do high yield dluhopisů investuje do:",
      options: [
        "Státních dluhopisů vyspělých zemí jako USA či Německo.",
        "Dluhopisů s ročním výnosem rovným výnosu příslušných státních dluhopisů.",
        "Dluhopisů s ročním výnosem minimálně 4 %",
        "Dluhopisů s kreditním ratingem ve spekulativní části ratingové stupnice.",
      ],
      answer:
        "Dluhopisů s kreditním ratingem ve spekulativní části ratingové stupnice.",
    },
    {
      question: "Jaký fond z výběru bude mít největší volatilitu?",
      options: [
        "Fond peněžního trhu",
        "Akciový fond",
        "Fond drahých kovů",
        "Dluhopisový fond",
      ],
      answer: "Akciový fond",
    },
    {
      question: "Jaký fond z výběru bude mít největší riziko?",
      options: [
        "Fond peněžního trhu",
        "Akciový fond",
        "Fond Bitcoin ETF",
        "Dluhopisový fond",
      ],
      answer: "Fond Bitcoin ETF",
    },
    {
      question: "Jaký fond z výběru bude mít pravděpodobně nejvyšší výnos?",
      options: [
        "Nemovitostní fond",
        "Dluhopisový fond",
        "Fond peněžního trhu",
        "Fond drahých kovů",
      ],
      answer: "Nemovitostní fond",
    },
    {
      question: "Jaký fond z výběru bude mít zpravidla nejvyšší poplatky?",
      options: [
        "Fond peněžního trhu s aktivní správou",
        "Fond korporátních dluhopisů",
        "Pasivně spravovaný nemovitostní fond",
        "Fond aktivně spravovaných akciových fondů",
      ],
      answer: "Fond aktivně spravovaných akciových fondů",
    },
    {
      question: "Jaký je hlavní problém investice do fondu fondů?",
      options: [
        "Vysoká volatilita",
        "Vysoké riziko",
        "Vysoká poplatková zátěž",
        "Nízká likvidita",
      ],
      answer: "Vysoká poplatková zátěž",
    },
    {
      question: "Co jsou to smíšené fondy?",
      options: [
        "Fondy, které jsou určené jak pro kvalifikované, tak pro začínající investory",
        "Fondy, do nichž můžou investovat investoři z různých zemí",
        "Fondy, do nichž investují jak jednotliví investoři, tak institucionální investoři",
        "Fondy, které investují do různých aktiv - akcií, dluhopisů, nemovitostí, komodit, atd.",
      ],
      answer:
        "Fondy, které investují do různých aktiv - akcií, dluhopisů, nemovitostí, komodit, atd.",
    },
    {
      question: "Co je to tzv. kyblíková metoda",
      options: [
        "Způsob rozložení portfolia do 3 pomyslných kyblíků dle rizikovosti",
        "Způsob rozložení portfolia do 3 pomyslných kyblíků dle likvidity",
        "Způsob rozložení portfolia do 3 pomyslných kyblíků dle výnosu",
        "Způsob rozložení portfolia do 3 pomyslných kyblíků dle způsobu správy",
      ],
      answer:
        "Způsob rozložení portfolia do 3 pomyslných kyblíků dle rizikovosti",
    },
    {
      question: "Co je to tzv. teritoriálně zaměřená investiční strategie?",
      options: [
        "Strategie, která investuje pouze do menších společností",
        "Strategie, která se soustředí na investice do různých sektorů průmyslu",
        "Strategie podporující pouze domácí ekonomiku",
        "Strategie, která investuje do společností, které působí v konkrétním regionu",
      ],
      answer:
        "Strategie, která investuje do společností, které působí v konkrétním regionu",
    },
    {
      question: "Co jsou to fondy indexů akciového trhu?",
      options: [
        "Fondy, které kopírují soubor akcií (index)",
        "Fondy, které jsou hodnocené podle jejich výkonnosti (indexovány) Českou národní bankou",
        "Fondy, které jsou pouze pro institucionální investory",
        "Fondy, které investují do akcií jen v období růstu trhu",
      ],
      answer: "Fondy, které kopírují soubor akcií (index)",
    },
    {
      question: "Co jsou to Fondy velkých hráčů (tzv. Blue chip)",
      options: [
        "Fondy, které investují do gamblerských společností",
        "Fondy, do kterých je možné investovat pouze větší částky",
        "Fondy, do kterých můžou investovat pouze institucionální investoři",
        "Fondy, které investují do největších společností na trhu",
      ],
      answer: "Fondy, které investují do největších společností na trhu",
    },
    {
      question: "Co jsou to sektorové fondy?",
      options: [
        "Fond, který sdružuje investory z určitého geografického regionu",
        "Fond zaměřený na investice do jednoho odvětví průmyslu",
        "Fond zaměřený na domácí trh",
        "Fond zaměřený na investice do jednoho regionu",
      ],
      answer: "Fond zaměřený na investice do jednoho odvětví průmyslu",
    },
    {
      question: "Co jsou to tzv. etické fondy?",
      options: [
        "Fondy, které se zaměřují pouze na hmotné investice (např. zlato či nemovitosti)",
        "Fondy, které investují na základě etických či náboženských principů a hodnot",
        "Fondy, které neinvestují do soukromého sektoru",
        "Fondy, které se vyhýbají jakémukoli riziku a investují pouze do bankovních vkladů",
      ],
      answer:
        "Fondy, které investují na základě etických či náboženských principů a hodnot",
    },
    {
      question:
        "Jaký typ fondů je akciový fond, ve kterém většinu tvoří tituly Pepsi, Microsoft, Apple, atd.?",
      options: [
        "Sektorový fond",
        "Etický fond",
        "Fond velkých hráčů",
        "Dividendový fond",
      ],
      answer: "Fond velkých hráčů",
    },
    {
      question: "Co jsou to dividendové fondy?",
      options: [
        "Fondy, ve kterých se každý akcionář podílí na investičních rozhodnutích",
        "Fondy, jejichž hodnota nemůže klesnout pod určitou hranici",
        "Fondy, které se primárně nezaměřují na růst ceny, ale na výplatu dividend (podíl na zisku společnosti)",
        "Fondy, které jsou dobře diverzifikované do různých sektorů průmyslu",
      ],
      answer:
        "Fondy, které se primárně nezaměřují na růst ceny, ale na výplatu dividend (podíl na zisku společnosti)",
    },
    {
      question: "Co jsou to fondy životního cyklu?",
      options: [
        "Fondy, které investují výhradně do startupů",
        "Fondy, které se zaměřují na investice do zdravotnického sektoru",
        "Fondy, které přizpůsobují svou investiční strategii věku investora, aby minimalizovaly riziko s blížícím se odchodem do důchodu",
        "Fondy, které investují pouze do akcií společností, které mají vysoký růstový potenciál",
      ],
      answer:
        "Fondy, které přizpůsobují svou investiční strategii věku investora, aby minimalizovaly riziko s blížícím se odchodem do důchodu",
    },
    {
      question: "Jaký z fondů není příliš populární?",
      options: ["Akciový", "Nemovitostní", "Devizový", "Dluhopisový"],
      answer: "Devizový",
    },
    {
      question:
        "Do jaké celkové částky odprodejů investičních aktiv během kalendářního roku nemusí poplatník danit?",
      options: ["100 000 Kč", "200 000 Kč", "500 000 Kč", "1 000 000 Kč"],
      answer: "100 000 Kč",
    },
    {
      question: "Co je to tzv. časový test?",
      options: [
        "Povinnost doložit při nákupu fondu, že investor má dostatečné finanční prostředky minimálně na tři roky nákupů dopředu",
        "Pravidlo, dle kterého je vhodnější investovat do fondu, který má minimálně tříletou historii",
        "Daňový test, podle kterého investor po třech letech nemusí danit zisk z prodeje podílových listů",
        "Pravidlo, dle kterého je vhodné kupovat pouze fondy, které v posledních třech letech zaznamenaly růst",
      ],
      answer:
        "Daňový test, podle kterého investor po třech letech nemusí danit zisk z prodeje podílových listů",
    },
    {
      question:
        "Jaké fondy jsou určené pro běžné investory s nižšími kapitálovými možnostmi?",
      options: [
        "Fondy pro institucionální investory",
        "Fondy kvalifikovaných investorů",
        "Mass retail fondy",
        "Fondy třetí kategorie",
      ],
      answer: "Mass retail fondy",
    },
    {
      question:
        "Jaká z následujících osob či institucí provádí investiční rozhodnutí v rámci fondu?",
      options: [
        "Manažer fondu",
        "Správce fondu",
        "Představenstvo fondu",
        "Akcionáři",
      ],
      answer: "Manažer fondu",
    },
    {
      question:
        "Jaká z následujících osob či institucí vlastní aktiva a mají právo na související příjmy?",
      options: [
        "Manažer fondu",
        "Správce fondu",
        "Představenstvo fondu",
        "Akcionáři",
      ],
      answer: "Akcionáři",
    },
    {
      question:
        "Mezi základní součásti řídícího a kontrolního systému obhospodařovatele investičního fondu nepatří:",
      options: [
        "Strategické a operativní řízení",
        "Systém řízení rizik",
        "Systém vnitřní kontroly",
        "Vytváření marketingových kampaní pro podporu prodeje",
      ],
      answer: "Vytváření marketingových kampaní pro podporu prodeje",
    },
    {
      question: "Do nákladů fondu kolektivního investování nelze zahrnout:",
      options: [
        "Náklady na audit účetní závěrky fondu",
        "Mzdové náklady portfolio manažera fondu",
        "Náklady na nabízení investic do tohoto fondu",
        "Úplatu za obhospodařování fondu",
      ],
      answer: "Náklady na nabízení investic do tohoto fondu",
    },
    {
      question: "Mezi investiční nástroje nepatří",
      options: [
        "Nástroje peněžního trhu",
        "Platební nástroje",
        "Investiční cenné papíry",
        "Cenné papíry kolektivního investování",
      ],
      answer: "Platební nástroje",
    },
    {
      question: "Pro fond kvalifikovaných investorů platí: ",
      options: [
        "Shromažďuje prostředky nebo jiná aktiva pouze od fyzických osob",
        "Shromažďuje prostředky nebo jiná aktiva od omezeného počtu investorů",
        "Může investovat pouze do akcií a dluhopisů",
        "Podléhá výrazně přísnější regulaci než fondy kolektivního investování",
      ],
      answer:
        "Shromažďuje prostředky nebo jiná aktiva od omezeného počtu investorů",
    },
  ],
  basics: [
    {
      question: "Co jsou to peníze?",
      options: [
        "Pouze mince a bankovky",
        "Dokument potvrzující majetkové právo",
        "Měna, která je zákonným platidlem",
        "Prostředek směny, uchovatel hodnoty a účetní jednotka",
      ],
      answer: "Prostředek směny, uchovatel hodnoty a účetní jednotka",
    },
    {
      question: "Co je to banka?",
      options: [
        "Instituce, která poskytuje finanční služby",
        "Vládní instituce, která přerozděluje peníze lidem",
        "Místo, kde se skladují fyzické peníze a zlato",
        "Systém pro sledování výdajů jednotlivců",
      ],
      answer: "Instituce, která poskytuje finanční služby",
    },
    {
      question: "Co je to výnos?",
      options: [
        "Rozdíl mezi náklady a získanými penězi",
        "Celková částka peněz na bankovním účtu",
        "Náklady spojené s provozováním podniku",
        "Peníze, které člověk může získat od banky",
      ],
      answer: "Rozdíl mezi náklady a získanými penězi",
    },
    {
      question: "Co je to riziko?",
      options: [
        "Možnost, že nastane nechtěná nebo nepředvídatelná událost s pozitivním dopadem na peníze",
        "Možnost, že nastane nechtěná nebo nepředvídatelná událost s negativním dopadem na peníze",
        "Možnost, že na základě pozitivních událostí získám peníze",
        "Množství peněz, které je potřeba investovat",
      ],
      answer:
        "Možnost, že nastane nechtěná nebo nepředvídatelná událost s negativním dopadem na peníze",
    },
    {
      question: "Co je to likvidita?",
      options: [
        "Schopnost rychle přeměnit aktiva na hotovost",
        "Úroveň úrokové sazby u investice",
        "Riziko spojené s investováním do akcií",
        "Množství peněz, které je třeba investovat",
      ],
      answer: "Schopnost rychle přeměnit aktiva na hotovost",
    },
    {
      question: "Co jsou to daně?",
      options: [
        "Trest za neplacení úroků soukromým osobám nebo firmám",
        "Povinné platby za poskytované služby (např. elektřina, voda, plyn)",
        "Povinné platby státu nebo místním orgánům na financování veřejných služeb",
        "Trest za trestnou činnost a porušení zákonů státu",
      ],
      answer:
        "Povinné platby státu nebo místním orgánům na financování veřejných služeb",
    },
    {
      question: "Co je to inflace?",
      options: [
        "Pokles cen zboží a služeb v ekonomice, který snižuje kupní sílu peněz",
        "Růst cen zboží a služeb v ekonomice, který snižuje kupní sílu peněz",
        "Pokles cen zboží a služeb v ekonomice, který zvyšuje kupní sílu peněz",
        "Růst cen zboží a služeb v ekonomice, který zvyšuje kupní sílu peněz",
      ],
      answer:
        "Růst cen zboží a služeb v ekonomice, který snižuje kupní sílu peněz",
    },
    {
      question: "Kolik ze základu 2000 Kč je 15%?",
      options: ["150 Kč", "200 Kč", "250 Kč", "300 Kč"],
      answer: "300 Kč",
    },
    {
      question: "Jaký obor se zabývá studiem peněz a jejich pohybu?",
      options: ["Matematika", "Politologie", "Ekonomie", "Sociologie"],
      answer: "Ekonomie",
    },
    {
      question: "Co je to HDP?",
      options: [
        "Harmonizovaný důchodový plán",
        "Historický daňový plán",
        "Hrubý domácí produkt",
        "Hlavní dividendový příjem",
      ],
      answer: "Hrubý domácí produkt",
    },
  ],
  alternativeInvestment: [],
  financeMaths: [
    {
      question: "Jak zpravidla uvádíme slevy produktů?",
      options: ["V procentech", "Ve zlomku", "V absolutním čísle", "Graficky"],
      answer: "V procentech",
    },
    {
      question:
        "Rychlovarná konvice stojí běžně 1 200 Kč. Je nyní ve slevě 20 %. Za kolik se dá koupit?",
      options: ["880 Kč", "960 Kč", "1000 Kč", "1040 Kč"],
      answer: "960 Kč",
    },
    {
      question:
        "Myčka stojí po slevě 10 000 Kč. Jaká byla původní cena, pokud byla sleva 20 %?",
      options: ["12 000 Kč", "12 500 Kč", "14 000 Kč", "15 000 Kč"],
      answer: "12 500 Kč",
    },
    {
      question:
        "Sada nádobí byla zlevněna o 15 % a pak kvůli nedostatku zájmu ještě o dalších 20%. Původní cena byla 3 000 Kč. Kolik stojí nyní?",
      options: ["1950 Kč", "2040 Kč", "2200 Kč", "2240 Kč"],
      answer: "2040 Kč",
    },
    {
      question: "Co jsou to falešné slevy?",
      options: [
        "Slevy, které se vztahují na zboží, které není v nabídce",
        "Slevy, které jsou uvedeny u produktu, ale obchodník je při platbě neuplatní",
        "Slevy, které jsou omezené časově, ale prodloužené kvůli poptávce",
        "Slevy, které jsou prezentovány jako vysoké, ale ve skutečnosti jsou pouze minimální nebo neexistují",
      ],
      answer:
        "Slevy, které jsou prezentovány jako vysoké, ale ve skutečnosti jsou pouze minimální nebo neexistují",
    },
    {
      question: "Co je to úrok?",
      options: [
        "Peníze, které musím zaplatit pokud poskytuji někomu půjčku",
        "Výnos z investice, který je vyplácen pravidelně",
        "Odměna poskytovateli peněz za to, že je půjčil",
        "Podíl na zisku společnosti",
      ],
      answer: "Odměna poskytovateli peněz za to, že je půjčil",
    },
    {
      question: "Na čem závisí, kolik peněz nakonec dostanu z úroku?",
      options: [
        "Pouze na množství peněz, které jsem půjčil",
        "Na úrokové míře, úročené částce, způsobu úročení a čase",
        "Na počtu vkladů, výši vkladu a likviditě vkladu",
        "Na výšce úrokové sazby a měně, ve které je vklad uložen",
      ],
      answer: "Na úrokové míře, úročené částce, způsobu úročení a čase",
    },
    {
      question: "Jaké způsoby úročení se běžně používají?",
      options: [
        "Jednoduché a složené úročení",
        "Inflační a kvadratické úročení",
        "Přímé a nepřímé úročení",
        "Jednoduché a kvadratické úročení",
      ],
      answer: "Jednoduché a složené úročení",
    },
    {
      question: "Co je to jednoduché úročení?",
      options: [
        "Úročení, kdy se úrok počítá dle aktuální míry inflace",
        "Úročení, kdy se úrok vypočítává z aktuální hodnoty vkladu včetně dříve připsaných úroků",
        "Úročení, kdy se úrok počítá na denní bázi",
        "Úročení, kdy se úrok počítá pouze z počáteční částky vkladu nebo půjčky",
      ],
      answer:
        "Úročení, kdy se úrok počítá pouze z počáteční částky vkladu nebo půjčky",
    },
    {
      question: "Co je to složené úročení?",
      options: [
        "Úročení, kdy se úrok počítá dle aktuální míry inflace a úrokové sazby",
        "Úročení, kdy se úrok vypočítává z aktuální hodnoty vkladu včetně dříve připsaných úroků",
        "Úročení, kdy se úrok počítá na denní bázi",
        "Úročení, kdy se úrok počítá pouze z počáteční částky vkladu nebo půjčky navýšené o fixní bonusovou částku",
      ],
      answer:
        "Úročení, kdy se úrok vypočítává z aktuální hodnoty vkladu včetně dříve připsaných úroků",
    },
    {
      question:
        "U jakého z následujících produktů se setkáme obvykle s jednoduchým úročením?",
      options: [
        "Investiční fond",
        "Spořící účet s průběžným úročením",
        "Stavební spoření s nízkorizikovým výnosem",
        "Termínovaný vklad s průběžnou platbou úroků na běžný účet",
      ],
      answer: "Termínovaný vklad s průběžnou platbou úroků na běžný účet",
    },
    {
      question: "Jaký vzorec se používá pro výpočet jednoduchého úročení?",
      options: [
        "Úrok = Hlavní jistina × (1 + úroková sazba)^počet období",
        "Úrok = Hlavní jistina ÷ úroková sazba × počet období",
        "Úrok = Hlavní jistina × úroková sazba ÷ počet období",
        "Úrok = Hlavní jistina × úroková sazba × počet období",
      ],
      answer: "Úrok = Hlavní jistina × úroková sazba × počet období",
    },
    {
      question:
        "Petr uložil 10 000 Kč na 3 roky s roční jednoduchou úrokovou sazbou 3%. Kolik dostane po 3 letech bez zdanění zpět?",
      options: ["10 300 Kč", "10 870 Kč", "10 900 Kč", "10 930 Kč"],
      answer: "10 900 Kč",
    },
    {
      question: "Jak se počítá složený úrok?",
      options: [
        "Úrok = Hlavní jistina × (úroková sazba)^(počet období)",
        "Úrok = Hlavní jistina × (1 + úroková sazba)^(počet období)",
        "Úrok = Hlavní jistina × (100 + úroková sazba)^(počet období)",
        "Úrok = Hlavní jistina × ((1 + úroková sazba)^(počet období) - 1)",
      ],
      answer:
        "Úrok = Hlavní jistina × ((1 + úroková sazba)^(počet období) - 1)",
    },
    {
      question:
        "V jakém poměru bude jednoduchý úrok k složenému úroku při stejné úrokové míře?",
      options: [
        "Jednoduchý a složený úrok budou stejné, jedná se jen o odlišnou metodu výpočtu",
        "Po jednom období bude jednoduchý úrok větší, po dvou obdobích bude složený úrok větší",
        "Po jednom a více obdobích bude složený úrok vždy větší nebo roven jednoduchému úroku",
        "Jednoduchý úrok bude vždy větší než složený úrok",
      ],
      answer:
        "Po jednom a více obdobích bude složený úrok vždy větší nebo roven jednoduchému úroku",
    },
    {
      question:
        "Petr uložil 10 000 Kč na 3 roky s roční složenou úrokovou sazbou 3%. Jaký bude čistý zisk po 3 letech bez zdanění?",
      options: ["300 Kč", "327 Kč", "900 Kč", "927 Kč"],
      answer: "927 Kč",
    },
    {
      question:
        "Podvodná investiční společnost tvrdí, že dovede zhodnotit peníze po delší dobu složeným úrokem 30% ročně. Kolikanásobek peněz by vám vrátila po třiceti letech, kdyby slib dodržela?",
      options: [
        "2,62 × výše vkladu",
        "26,2 × výše vkladu",
        "262 × výše vkladu",
        "2620 × výše vkladu",
      ],
      answer: "2620 × výše vkladu",
    },
    {
      question:
        "Mirek má na výběr mezi jednoduchým úročením 10% a 11% na 30 let. Jak velký procentuální rozdíl ve zisku bude?",
      options: [
        "Druhá možnost je o 9 % výhodnější, než první",
        "Druhá možnost je o 10 % výhodnější než první",
        "Druhá možnost je o 22 % výhodnější, než první",
        "Druhá možnost je o 31 % výdhonější, než první",
      ],
      answer: "Druhá možnost je o 10 % výhodnější, než první",
    },
    {
      question:
        "Mirek má na výběr mezi složeným úročením 10% a 11% na 30 let. Jak velký procentuální rozdíl ve zisku bude?",
      options: [
        "Druhá možnost je o 9 % výhodnější, než první",
        "Druhá možnost je o 10 % výhodnější než první",
        "Druhá možnost je o 22 % výhodnější, než první",
        "Druhá možnost je o 31 % výhodnější, než první",
      ],
      answer: "Druhá možnost je o 31 % výhodnější, než první",
    },
    {
      question:
        "Pokud si chceme uložit částku na 10 let se složeným úročením 5%, jak vysoký by musel být jednoduchý úrok, abychom získali stejné množství peněz?",
      options: ["8,2 %", "12,6 %", "16,3 %", "18,2 %"],
      answer: "16,3 %",
    },
    {
      question:
        "Akcie roste v průměru o 10% ročně. Jak vysoký je její průměrný pětiletý růst?",
      options: [
        "0,1 × 5 = 50 %",
        "1,1^5 - 1 = 61 %",
        "1,1 × 0,5 = 55 %",
        "1,1^5 = 161 %",
      ],
      answer: "1,1^5 - 1 = 61 %",
    },
    {
      question:
        "Spořící účet má složenou úrokovou stavbu 3% ročně. Jeho úročení ovšem probíhá denně. Jaký je tento reálný složený denní úrok?",
      options: [
        "1,03^(1/365) - 1 = 0,0081 %",
        "0,03/365 = 0,0082 %",
        "(3+1)/3650 = 0,11 %",
        "1,03/365 = 0,27 %",
      ],
      answer: "1,03^(1/365) - 1 = 0,0081 %",
    },
    {
      question: "Co je to geometrická posloupnost?",
      options: [
        "Posloupnost, kde každý člen je součtem předchozího členu a konstantního rozdílu",
        "Posloupnost, kde každý člen je součinem předchozího členu a konstantního poměru",
        "Posloupnost, kde každý člen je druhou mocninou předchozího členu",
        "Posloupnost, kde každý člen je rozdílem předchozího členu a konstantního rozdílu",
      ],
      answer:
        "Posloupnost, kde každý člen je součinem předchozího členu a konstantního poměru",
    },
    {
      question: "Která z následujících posloupností je geometrická?",
      options: [
        "2, 4, 6, 8, 10",
        "3, 9, 27, 81, 243",
        "1, 1, 2, 3, 5",
        "5, 10, 20, 35, 55",
      ],
      answer: "3, 9, 27, 81, 243",
    },
    {
      question:
        "Při jakém výpočtu se setkáváme s použitím geometrické posloupnosti?",
      options: [
        "Úroků se základním vkladem",
        "Spoření s průběžným vkladem",
        "Riziko investičního faktoru beta",
        "Odhad ceny dluhopisu",
      ],
      answer: "Spoření s průběžným vkladem",
    },
  ],
  financeLiteracy: [
    {
      question: "S čím nám může finanční gramotnost pomoct?",
      options: [
        "S větší orientací na přítomnost",
        "S porozuměním státního rozpočtu",
        "S dosažením finančních cílů v budoucnosti",
        "S porozuměním složitějších matematických konceptů",
      ],
      answer: "S dosažením finančních cílů v budoucnosti",
    },
    {
      question:
        "Jaká z následujících částí se běžně neuvádí jako součást finanční gramotnosti?",
      options: ["Peněžní", "Investiční", "Cenová", "Rozpočtová"],
      answer: "Investiční",
    },
    {
      question: "Proč existují peníze?",
      options: [
        "Peníze existují jako směnný prostředek bankovního sektoru",
        "Peníze slouží jako prostředek dosažení osobních cílů bohatých",
        "Peníze existují primárně kvůli zaměstnávání lidí podnikateli",
        "Peníze existují jako prostředek směny a uchovatel hodnoty",
      ],
      answer: "Peníze existují jako prostředek směny a uchovatel hodnoty",
    },
    {
      question:
        "Jaká z hodnot je nejvíc spojená s vlastnictvím peněz z pohledu finanční gramotnosti?",
      options: ["Nezávislost", "Moc", "Rovnost", "Bezpečí"],
      answer: "Nezávislost",
    },
    {
      question: "Jaký je inflační cíl České národní banky?",
      options: ["1%", "2%", "3%", "4%"],
      answer: "2%",
    },
    {
      question: "Jaký má vliv inflace na hodnotu peněz?",
      options: [
        "Hodnota peněz se zvyšuje s inflací",
        "Hodnota peněz se snižuje s inflací",
        "Hodnota peněz nezávisí na inflaci",
        "Hodnota peněz je vždy stejná",
      ],
      answer: "Hodnota peněz se snižuje úměrně inflaci",
    },
    {
      question:
        "Jak se nazývají peníze, které mají hodnotu jen díky rozhodnutí státu?",
      options: [
        "Fiat peníze",
        "Barterové peníze",
        "Samozřejmé peníze",
        "Přirozené peníze",
      ],
      answer: "Fiat peníze",
    },
    {
      question:
        "Jsou v současnosti peníze v České republice kryté zlatem či jinou hodnotnou komoditou?",
      options: ["Ano", "Ne"],
      answer: "Ne",
    },
    {
      question: "Slouží fiat peníze jako dlouhodobý způsob uchování hodnoty?",
      options: ["Ano", "Ne"],
      answer: "Ne",
    },
    {
      question: "O kolik % se snížila kupní síla peněz mezi lety 2014 a 2024?",
      options: ["O 5%", "O 15%", "O 25%", "O 35%"],
      answer: "O 35%",
    },
    {
      question:
        "Při zachování inflačníh ocíle 2% by měla poklesnout kupní síla peněz za 10 let o:",
      options: ["Asi 9%", "Asi 12%", "Asi 15%", "Asi 18%"],
      answer: "Asi 18%",
    },
    {
      question:
        "Jaká z následujících metod není vhodná k úplné ani částečné ochraně peněz před inflací?",
      options: [
        "Spoření v deflační měně (např. Bitcoin)",
        "Držení peněz v hotovosti",
        "Využíváním spořících produktů ",
        "Investováním na finančních trzích",
      ],
      answer: "Držení peněz v hotovosti",
    },
    {
      question: "Jaký je rozdíl mezi spořením a investováním?",
      options: [
        "Při investic vystavuji peníze riziku ztráty, u spoření ne",
        "Spoření a investování mají stejný význam",
        "Investování je jen pro bohaté, spoření pro běžné lidi",
        "Spoření je více ovlivněno krizemi finančního trhu",
      ],
      answer: "Při investic vystavuji peníze riziku ztráty, u spoření ne",
    },
    {
      question: "Dosahuje zhodnocení spořících produktů inflace?",
      options: ["Obvykle ne", "Obvykle ano"],
      answer: "Obvykle ne",
    },
    {
      question: "Co jsou to nízké a vysoké časové preference?",
      options: [
        "Preferování dlouhých a krátkých časových horizontů při investování",
        "Rozdíl mezi časem stráveným prací a volným časem",
        "Přístup lidí k penězům z hlediska diskontování spotřeby v budoucnosti",
        "Výběr mezi investicemi s nízkým a vysokým rizikem",
      ],
      answer:
        "Přístup lidí k penězům z hlediska diskontování spotřeby v budoucnosti",
    },
    {
      question: "Člověk, který má vysokou časovou preferenci, bude vykazovat:",
      options: [
        "Tendenci upřednostňovat okamžitou spotřebu před budoucími úsporami",
        "Tendenci investovat dlouhodobě do stabilních aktiv",
        "Silnou schopnost odkládat spotřebu a šetřit",
        "Zájem o nízkorizikové investice",
      ],
      answer:
        "Tendenci upřednostňovat okamžitou spotřebu před budoucími úsporami",
    },
    {
      question: "Člověk, který má nízkou časovou preferenci, bude vykazovat:",
      options: [
        "Zájem o investice převážně do technologických společností",
        "Tendenci diskontovat budoucí spotřebu jen mírně",
        "Tendenci utrácet v přítomnosti a nešetřit",
        "Zájem o investice s vyšším rizikem",
      ],
      answer: "Tendenci diskontovat budoucí spotřebu jen mírně",
    },
    {
      question: "Proč existují časové preference?",
      options: [
        "Člověk má tendenci odkládat rozhodnutí",
        "Člověk je smrtelný a čelí tedy omezenému času, upřednostňuje tedy okamžitou spotřebu",
        "Člověk je víceméně racionální a vždy upřednostňuje budoucí spotřebu",
        "Člověk má menší motivaci k okamžité spotřebě než k budoucí spotřebě",
      ],
      answer:
        "Člověk je smrtelný a čelí tedy omezenému času, upřednostňuje tedy okamžitou spotřebu",
    },
    {
      question:
        "Jak obecně souvisí míra zdanění a inflace s časovými preferencemi?",
      options: [
        "Vyšší zdanění a vyšší inflace přispívá k vysokým časovým preferencím",
        "Vyšší zdanění a nižší inflace přispívá k vysokým časovým preferencím",
        "Nižší zdanění a vyšší inflace přispívá k vysokým časovým preferencím",
        "Nižší zdanění a nižší inflace přispívá k vysokým časovým preferencím",
      ],
      answer:
        "Vyšší zdanění a vyšší inflace přispívá k vysokým časovým preferencím",
    },
    {
      question:
        "Jaké z následujících chování je typické pro lidi s nízkými časovými preferencemi?",
      options: [
        "Konzumace alkoholu či tabákových výrobků ve větší míře",
        "Vyhýbání se dlouhodobým investicím",
        "Udržování zdravého životního stylu",
        "Zvýšená konzumace a impulzivní nakupování",
      ],
      answer: "Udržování zdravého životního stylu",
    },
    {
      question: "Proč některá zaměstnání vydělávají více než jiná?",
      options: [
        "Protože práce mají stejnou časovou hodnotu, ale zaměstnavatel vždy určitou část této hodnoty zadrží pro sebe",
        "Platové třídy jsou nastavené vládou a zaměstnavatelé se musí řídit těmito pravidly",
        "Náročnější zaměstnání je vždy lépe placené",
        "Protože nabídka a poptávka na trhu práce určují cenu práce",
      ],
      answer: "Protože nabídka a poptávka na trhu práce určují cenu práce",
    },
    {
      question: "Co můžu udělat pro zvýšení své hodnoty na trhu práce?",
      options: [
        "Zvyšovat své dovednosti a kvalifikace prostřednictvím vzdělávání",
        "Být více závislý na sobě a vyhýbat se týmové práci",
        "Zvýšit své pracovní nasazení",
        "Omezit své pracovní zkušenosti na jedno odvětví a jednu pozici",
      ],
      answer:
        "Zvyšovat své dovednosti a kvalifikace prostřednictvím vzdělávání",
    },
    {
      question:
        "Jaké z následujících životních rozhodnutí vás zpravidla nejméně ovlivní?",
      options: [
        "Rozhodnutí o založení rodiny",
        "Koupě auta",
        "Výběr kariéry a vzdělání",
        "Koupě nemovitosti",
      ],
      answer: "Koupě auta",
    },
    {
      question: "Jaká je průměrná hrubá mzda v roce 2024?",
      options: ["29 000 Kč", "35 000 Kč", "41 000 Kč", "44 000 Kč"],
      answer: "44 000 Kč",
    },
    {
      question: "Jaká je aktuálně minimální mzda v České republice?",
      options: ["16 400 Kč", "17 200 Kč", "18 900 Kč", "20 500 Kč"],
      answer: "18 900 Kč",
    },
    {
      question: "Jaký je průměrný důchod v roce 2024?",
      options: ["18 900 Kč", "20 700 Kč", "22 300 Kč", "24 000 Kč"],
      answer: "20 700 Kč",
    },
    {
      question:
        "Jaká z následujících pozic má zpravidla nejnižší průměrný plat?",
      options: [
        "Daňový poradce",
        "Středoškolský pedagog",
        "Instalatér",
        "IT freelancer",
      ],
      answer: "Středoškolský pedagog",
    },
    {
      question: "Jaký ze zmíněných států má nejvyšší průměrnou mzdu?",
      options: ["Rakousko", "Česká republika", "Katar", "Bhútán"],
      answer: "Katar",
    },
    {
      question: "Co je to průměrná kupní síla občana a na čem závisí?",
      options: [
        "Schopnost občana ovlivnit ceny na trhu, závisí na jeho výdajích",
        "Průměrná hodnota peněz, které občan ušetří, závisí na jeho úsporách",
        "Průměrná hodnota majetku občana, závisí na jeho investicích a likvidním kapitálu",
        "Hodnota peněz, které člověk získá, závisí jednak na průměrné mzdě, jednak na cenové hladině",
      ],
      answer:
        "Hodnota peněz, které člověk získá, závisí jednak na průměrné mzdě, jednak na cenové hladině",
    },
    {
      question:
        'Jakých faktických chyb se dopustil autor této teze? "Za bývalého režimu bylo vše levnější, např. pivo stálo 2 Kčs. Teď stojí dvacetinásobek. Proto se žilo lépe."',
      options: [
        "Tvrzení je chybné, protože autor opomněl změny v kvalitě výrobků a jejich vliv na ceny",
        "Tvrzení je pravdivé, protože ceny stoupají a kupní síla klesá díky inflaci",
        "Tvrzení je chybné, protože autor nezohlednil růst reálné mzdy, který kompenzuje růst cen způsobený inflací",
        "Tvrzení je pravdivé, protože životní úroveň byla vyšší za bývalého režimu",
      ],
      answer:
        "Tvrzení je chybné, protože autor nezohlednil růst reálné mzdy, který kompenzuje růst cen způsobený inflací",
    },
    {
      question: "Co je to den daňové svobody?",
      options: [
        "Den, od kterého pracující člověk po dosažení určitého věku není zatížen daněmi",
        "Den, kdy zaměstnanci neplatí sociální a zdravotní pojištění",
        "Ukazatel, který určuje, jak dlouho z roku pracujeme pro stát, tedy daňovou zátěž",
        "Dny daňové svobody jsou ty dny, kdy podnikatel nemusí danit zisk",
      ],
      answer:
        "Ukazatel, který určuje, jak dlouho z roku pracujeme pro stát, tedy daňovou zátěž",
    },
    {
      question: "Kdy je v České republice den daňové svobody (K roku 2024)?",
      options: ["20. dubna", "4. května", "11. června", "15. července"],
      answer: "11. června",
    },
    {
      question: "Jaký z následujících států má nejvyšší daňovou zátěž?",
      options: ["Česká republika", "Švýcarsko", "Rakousko", "Slovensko"],
      answer: "Rakousko",
    },
    {
      question: "Co je to věková inflace?",
      options: [
        "Jev, kdy inflace ovlivňuje různé věkové skupiny odlišně",
        "Jev, kdy se s věkem zvyšuje kupní síla peněz",
        "Jev, kdy subjektivní hodnota peněz klesá s věkem kvůli růstu příjmu",
        "Jev, který ovlivňuje věk odchodu do důchodu",
      ],
      answer:
        "Jev, kdy subjektivní hodnota peněz klesá s věkem kvůli růstu příjmu",
    },
    {
      question:
        "Jak moc jsou Češi ochotní podstupovat finanční rizika spojená s investicemi?",
      options: [
        "Češi patří k podstupování rizika ke světovému průměru. Přístup k riziku se obecně napříč státy moc neliší",
        "Ačkoliv jsou Češi málo ochotní podstupovat riziko, podstupují ho radši než např. Němci",
        "Češi (podobně jako občané ostatních postsovětských zemí) mají k riziku značnou averzi",
        "Češi mají vysokou toleranci k riziku, pouštějí se i do velmi rizikových investic",
      ],
      answer:
        "Češi (podobně jako občané ostatních postsovětských zemí) mají k riziku značnou averzi",
    },
  ],
  budgeting: [
    {
      question: "Co nejlépe vystihuje budgeting?",
      options: [
        "Dovednost dělat důležitá podnikatelská rozhodnutí",
        "Dovednost orientace na finančních trzích",
        "Dovednost tvorby osobního rozpočtu",
        "Dovednost porozumění teoretické ekonomii",
      ],
      answer: "Dovednost tvorby osobního rozpočtu",
    },
    {
      question: "Jaká by měla být základní struktura osobního rozpočtu?",
      options: [
        "Dělení podle možného výnosu na spořící a aktivní rezervu",
        "Dělení podle množství peněz na malou, střední a velkou peněžní rezervu",
        "Dělení podle rizika na bezrizikové, středně rizikové, značně rizikové (a gamble) rezervu",
        "Dělení podle likvidity na krátkodobou, střednědobou a dlouhodobou rezervu",
      ],
      answer:
        "Dělení podle likvidity na krátkodobou, střednědobou a dlouhodobou rezervu",
    },
    {
      question: "Co nejlépe charakterizuje prostředky krátkodobé rezervy",
      options: [
        "Prostředky s vysokou či okamžitou likviditou a nízkým rizikem",
        "Investice do dlouhodobých aktiv, jako jsou nemovitosti",
        "Půjčka s krátkou dobou splatnosti",
        "Peněžní prostředky určené na důchodové spoření",
      ],
      answer: "Prostředky s vysokou či okamžitou likviditou a nízkým rizikem",
    },
    {
      question: "Na co se krátkodobá rezerva zpravidla používá?",
      options: [
        "Odkládání peněz s vysokou likviditou pro případ velkých nečekaných nákladů",
        "Na koupi nového auta, rekonstrukci domácnosti či jiné středně rozsáhlé platby",
        "Na nižší nečekané výdaje, okamžité platby a pokrytí krátkého výpadku příjmů",
        "Na odkládání přebytečných finančních prostředků s vyšší likviditou na penzi",
      ],
      answer:
        "Na nižší nečekané výdaje, okamžité platby a pokrytí krátkého výpadku příjmů",
    },
    {
      question: "V jakých aktivech by měla být uložena krátkodobá rezerva?",
      options: [
        "Termínované vklady, stavební spoření, hotovost",
        "Akcie, dluhopisy, nemovitosti",
        "Kryptoměny, akcie, dluhopisy",
        "Spořící účty, běžné účty, hotovost",
      ],
      answer: "Spořící účty, běžné účty, hotovost",
    },
    {
      question: "Jak vysoká by měla být krátkodobá rezerva",
      options: [
        "Alespoň 10 000 Kč",
        "Alespoň 50 000 Kč",
        "1 -2 měsíční výdaje",
        "3 - 6 měsíčních výdajů",
      ],
      answer: "3 - 6 měsíčních výdajů",
    },
    {
      question: "Co nejlépe charakterizuje prostředky střednědobé rezervy",
      options: [
        "Prostředky s nízkou likviditou a vysokou volatilitou",
        "Prostředky s vysokou likviditou a volatilitou",
        "Prostředky se střední likviditou a volatilitou",
        "Prostředky s vysokou likviditou a nízkou volatilitou ",
      ],
      answer: "Prostředky se střední likviditou a volatilitou",
    },
    {
      question: "Na co se střednědobá rezerva zpravidla používá?",
      options: [
        "Odkládání peněz s vysokou likviditou pro případ velkých nečekaných nákladů",
        "Na koupi nového auta, rekonstrukci domácnosti či jiné středně rozsáhlé platby",
        "Na nižší nečekané výdaje, okamžité platby a pokrytí krátkého výpadku příjmů",
        "Na odkládání přebytečných finančních prostředků s vyšší likviditou na penzi",
      ],
      answer:
        "Na koupi nového auta, rekonstrukci domácnosti či jiné středně rozsáhlé platby",
    },
    {
      question: "V jakých aktivech by měla být uložena střednědobá rezerva?",
      options: [
        "Termínované vklady, stavební spoření, hotovost",
        "Méně rizikové akciové fondy, dluhopisové fondy",
        "Kryptoměny, akcie, dluhopisy",
        "Spořící účty, běžné účty, hotovost",
      ],
      answer: "Méně rizikové akciové fondy, dluhopisové fondy",
    },
    {
      question: "Jak vysoká by měla být střednědobá rezerva",
      options: [
        "Střednědobá rezerva by měla tvořit hlavní položku osobních aktiv",
        "Úměrně krátkodobé finanční rezervě, ne však zbytečně moc",
        "3 - 6 měsíčních výdajů",
        "6 - 12 měsíčních výdajů",
      ],
      answer: "Úměrně krátkodobé finanční rezervě, ne však zbytečně moc",
    },
    {
      question: "Co nejlépe charakterizuje prostředky dlouhodobé rezervy",
      options: [
        "Prostředky s nízkou likviditou a vysokou volatilitou",
        "Prostředky s vysokou likviditou a volatilitou",
        "Prostředky se střední likviditou a volatilitou",
        "Prostředky s vysokou likviditou a nízkou volatilitou ",
      ],
      answer: "Prostředky s nízkou likviditou a vysokou volatilitou",
    },
    {
      question: "Na co se dlouhodobá rezerva zpravidla používá?",
      options: [
        "Odkládání penězŠ s vysokou likviditou pro případ velkých nečekaných nákladů",
        "Na koupi nového auta, rekonstrukci domácnosti či jiné středně rozsáhlé platby",
        "Na velké projekty jako koupě nemovitosti a k zajištění životního standardu v penzi",
        "Na odkládání přebytečných finančních prostředků s vyšší likviditou na penzi",
      ],
      answer:
        "Na velké projekty jako koupě nemovitosti a k zajištění životního standardu v penzi",
    },
    {
      question: "V jakých aktivech by měla být uložena dlouhodobá rezerva?",
      options: [
        "Termínované vklady, stavební spoření, hotovost",
        "Méně rizikové akciové fondy, dluhopisové fondy",
        "Akciové fondy a indexy, korporátní dluhopisy",
        "Spořící účty, běžné účty, hotovost",
      ],
      answer: "Akciové fondy a indexy, korporátní dluhopisy",
    },
    {
      question: "Jak vysoká by měla být dlouhodobá rezerva",
      options: [
        "Do 1 000 000 Kč, další prostředky je lepší uchovávat ve střednědobé a krátkodobé rezervě",
        "Co nejvyšší za předpokladu, že je střednědobá a krátkodobá rezerva zajištěna",
        "12 - 24 měsíčních výdajů",
        "24 - 48 měsíčních výdajů",
      ],
      answer:
        "Co nejvyšší za předpokladu, že je střednědobá a krátkodobá rezerva zajištěna",
    },
    {
      question:
        "Jak v případě aktuálního finančního deficitu zlepšit svou finanční situaci?",
      options: [
        "Spotřebitelským úvěrem u banky",
        "Začít s investicemi do akcií a nemovitostních fondů",
        "Pokusit se zvýšit příjmy a snížit výdaje",
        "Založit finanční produkty jako stavební spoření nebo termínovaný vklad",
      ],
      answer: "Pokusit se zvýšit příjmy a snížit výdaje",
    },
    {
      question: "Který výdaj z výběru neřadíme mezi fixní výdaje?",
      options: [
        "Nájem",
        "Splátka hypotéky",
        "Předplatné oblíbené služby",
        "Jídlo v restauraci",
      ],
      answer: "Jídlo v restauraci",
    },
    {
      question:
        "Jaká je vhodná dlouhodobá strategie pro osobu, která obvykle všechny peníze z výplaty utratí a neušetří?",
      options: [
        "Využívání bankovních produktů jako jsou spotřebitelský úvěr, získané finance investovat",
        "Zvýšit příjmy a udržet si životní styl",
        "Odkládání části peněz na spořící účet a investování malých částek do dlouhodobých aktiv",
        "Založení penzijního spoření a nechat si od zaměstnavatele posílat část peněz tam",
      ],
      answer:
        "Odkládání části peněz na spořící účet a investování malých částek do dlouhodobých aktiv",
    },
  ],
  financeMarkets: [
    {
      question: "Co jsou to burzy?",
      options: [
        "Finanční instituce podobné bankám, které uchovávají cenné papíry",
        "Instituce vlastněné státem pro obchodování s finančními aktivy",
        "Instituce, které přerozdělují majetek ve společnosti ve prospěch bohatých",
        "Tržní instituce, kde se obchoduje s cennými papíry, komoditami a dalšími finančními nástroji",
      ],
      answer:
        "Organizované trhy, kde se obchoduje s cennými papíry, komoditami a dalšími finančními nástroji",
    },
    {
      question: "Proč existují burzy?",
      options: [
        "Pro kontrolu cenové hladiny v ekonomice",
        "Pro efektivní obchodování s finančními nástroji odpovídající nabídce a poptávce",
        "Pro umožnění bankám a bohatým lidem bohatnout na úrkor ostatních",
        "Pro řízení měnové politiky země",
      ],
      answer:
        "Pro efektivní obchodování s finančními nástroji odpovídající nabídce a poptávce",
    },
    {
      question: "Jaké z následujících aktiv běžně neobchodujeme na burze?",
      options: ["Akcie", "Dluhopisy", "Komodity", "Nemovitosti"],
      answer: "Nemovitosti",
    },
    {
      question: "Co je to investiční portfolio?",
      options: [
        "Tržní hodnota všech akcií v ekonomice",
        "Sestava investičních aktiv, které investor vlastní",
        "Dokument obsahující investiční strategie společnosti",
        "Přehled veškerého majetku investora",
      ],
      answer: "Sestava investičních aktiv, které investor vlastní",
    },
    {
      question: "Co je to dluhopis?",
      options: [
        "Forma akcie, která zajišťuje podíl na zisku společnosti",
        "Krátkodobý finanční nástroj s vysokým rizikem",
        "Půjčka, kterou si investor může vzít od emitenta",
        "Cenný papír, který představuje půjčku poskytnutou investorem emitentovi",
      ],
      answer:
        "Cenný papír, který představuje půjčku poskytnutou investorem emitentovi",
    },
    {
      question:
        "Emitentem dluhopisu může být za předpokladu splnění všech podmínek",
      options: [
        "Jakákoliv právnická nebo fyzická osoba",
        "Pouze obchodní společnost",
        "Pouze územní samosprávný celek (kraj, obec, stát)",
        "Pouze banka či jiná finanční instituce",
      ],
      answer: "Jakákoliv právnická nebo fyzická osoba",
    },
    {
      question: "Co je to akcie?",
      options: [
        "Cenný papír, který představuje půjčku poskytnutou investorem emitentovi",
        "Bezrizikový finanční nástroj s proměnlivým úrokem",
        "Cenný papír, který představuje podíl na majetku a zisku emitenta",
        "Forma korporátního dluhopisu s pevnou úrokovou sazbou",
      ],
      answer:
        "Cenný papír, který představuje podíl na majetku a zisku emitenta",
    },
    {
      question: "Proč společnost vydává akcie?",
      options: [
        "Aby mohla regulovat svou provozní činnost",
        "Aby snížila své daňové zatížení",
        "Aby snížila svůj dluh",
        "Aby získala kapitál na financování projektů a rozvoje",
      ],
      answer: "Aby získala kapitál na financování projektů a rozvoje",
    },
    {
      question:
        'Je toto tvrzení pravdivé? "Když si koupím akcii na burze, podporuji přímo finančně firmu, která ji vydala"',
      options: [
        "Ano, protože každý nákup akcií poskytuje firmě nový kapitál",
        "Ne, protože akcie se zpravidla obchodují s jinými investory",
      ],
      answer: "Ne, protože akcie se zpravidla obchodují s jinými investory",
    },
    {
      question:
        "Jak se nazývá projevení zájmu investora o nově emitované akcie?",
      options: [
        "Upisování",
        "Opce",
        "Emise",
        "Obchodování na sekundárním trhu",
      ],
      answer: "Upisování",
    },
    {
      question: "Kdo získává peníze při upisování akcií?",
      options: ["Investor", "Emitent", "Stát", "Banka"],
      answer: "Emitent",
    },
    {
      question: "Co je to komodita?",
      options: [
        "Měnový poukázka opravňující ke směně za jinou měnu v předem daném kurzu",
        "Fyzické suroviny nebo produkty, které jsou obchodovány na trzích, jako jsou zlato, ropa, pšenice",
        "Podíl na hmotném majetku společnosti (nemovitosti, výrobní prostředky...)",
        "Podíl na nehmotném majetku společnosti (značka, know-how...)",
      ],
      answer:
        "Fyzické suroviny nebo produkty, které jsou obchodovány na trzích, jako jsou zlato, ropa, pšenice",
    },
    {
      question: "Jaká je nejstarší burza na světě?",
      options: [
        "New York Stock Exchange (NYSE)",
        "London Stock Exchange (LSE)",
        "Amsterdam Stock Exchange",
        "Frankfurt Stock Exchange",
      ],
      answer: "Amsterdam Stock Exchange",
    },
    {
      question: "V jakém roce vznikla první burza?",
      options: ["1602", "1702", "1802", "1902"],
      answer: "1602",
    },
    {
      question:
        "Jaká je největší burza na světě dle objemu obchodovaného kapitálu?",
      options: [
        "New York Stock Exchange (NYSE)",
        "London Stock Exchange (LSE)",
        "Tokyo Stock Exchange (TSE)",
        "Shanghai Stock Exchange",
      ],
      answer: "New York Stock Exchange (NYSE)",
    },
    {
      question: "Čím se řídí ceny finančních instrumentů na burze?",
      options: [
        "Státní regulací",
        "Dohodou mezi velkými investory",
        "Nabídkou a poptávkou",
        "Dle obratu emitenta",
      ],
      answer: "Nabídkou a poptávkou",
    },
    {
      question:
        "Jaká z následujících věcí neovlivňuje cenu finančního instrumentu?",
      options: [
        "Nálada investorů",
        "Makroekonomické a mikroekonomické faktory",
        "Geopolitika",
        "Historie ceny produktu v jiném odvětví",
      ],
      answer: "Historie ceny produktu v jiném odvětví",
    },
    {
      question: "Může na burzách jako je například NYSE obchodovat kdokoliv?",
      options: ["Ano", "Ne"],
      answer: "Ne",
    },
    {
      question:
        "Kde může obchodovat běžný člověk s akciemi či jinými cennými aktivy?",
      options: [
        "Pouze prostřednictvím banky, ostatní služby jsou nedůvěryhodné",
        'Prostřednictvím banky či "brokera"',
        "Běžný investor si musí obstarat licenci pro obchodování s cennými papíry",
        "Běžný investor nemá přístup k obchodování s cennými papíry",
      ],
      answer: 'Prostřednictvím banky či "brokera"',
    },
    {
      question:
        "Burzovní obchody dělíme i z hlediska délky vypořádání obchodů. Spotové obchody jsou:",
      options: [
        "Vypořádány v řádu dnů",
        "Vypořádány v řádu týdnů",
        "Vypořádány v řádu měsíců",
        "Vypořádání v řádu let",
      ],
      answer: "Vypořádány v řádu dnů",
    },
    {
      question:
        "V souvislosti s tvorbou investičního portfolia neplatí tato zásada:",
      options: [
        "Vhodně provedená diverzifikace snižuje systematické riziko portfolia",
        "Čím bude menší vzájemná závislost mezi změnami výnosnosti jednotlivých investičních nástrojů, tím větší omezení rizikovosti diverzifikace přinese",
        "Rizikovost portfolia klesá s počtem do něj zařazených druhů investičních instrumentů, a to bez ohledu na to, o jak rizikové nástroje se jedná",
        "Rizikovost portfolia lze snížit buď zařazením méně rizikových investičních nástrojů, nebo vyšší vahou, jež jim bude v portfoliu přidělena.",
      ],
      answer:
        "Rizikovost portfolia klesá s počtem do něj zařazených druhů investičních instrumentů, a to bez ohledu na to, o jak rizikové nástroje se jedná",
    },
    {
      question:
        "Jaký z následujících faktorů nepatří do tzv. investičního trojúhelníku?",
      options: ["Riziko", "Likvidita", "Udržitelnost", "Výnos"],
      answer: "Udržitelnost",
    },
    {
      question: "Co je to konjunktura?",
      options: [
        "Období ekonomického poklesu",
        "Období ekonomického růstu a prosperity",
        "Fáze obchodního cyklu charakterizovaná vysokou nezaměstnaností",
        "Období stagnace ekonomiky",
      ],
      answer: "Období ekonomického růstu a prosperity",
    },
    {
      question: "Co je to recese?",
      options: [
        "Období ekonomického poklesu trvající alespoň dvě po sobě jdoucí čtvrtletí",
        "Období extrémního ekonomického růstu",
        "Fáze obchodního cyklu charakterizovaná vysokou nezaměstnaností",
        "Období stabilní ekonomiky bez růstu nebo poklesu",
      ],
      answer:
        "Období ekonomického poklesu trvající alespoň dvě po sobě jdoucí čtvrtletí",
    },
    {
      question: "Pokud trh stoupá, mluvíme o:",
      options: ["Medvědím trhu", "Jelením trhu", "Býčím trhu", "Tygřím trhu"],
      answer: "Býčím trhu",
    },
    {
      question: "Pokud trh klesá, mluvíme o:",
      options: ["Medvědím trhu", "Jelením trhu", "Býčím trhu", "Tygřím trhu"],
      answer: "Medvědím trhu",
    },
    {
      question: "Jak moc ceny na trhu závisí na náladě investorů?",
      options: [
        "Nálada investorů má malý vliv na ceny na trhu",
        "Nálada investorů ovlivňuje pouze krátkodobé výkyvy cen",
        "Nálada investorů může významně ovlivnit ceny na trhu",
        "Nálada investorů nemá žádný vliv na ceny na trhu",
      ],
      answer: "Nálada investorů může významně ovlivnit ceny na trhu",
    },
    {
      question: "Jaké emoce nejvíce ovlivňují investory na burze?",
      options: [
        "Strach a chamtivost",
        "Lhostejnost a zájem",
        "Radost a smutek",
        "Zodpovědnost a apatie",
      ],
      answer: "Strach a chamtivost",
    },
    {
      question: "Kolik procent investorů na finančních trzích prodělává?",
      options: ["10 - 30 %", "30 - 50 %", "50 - 70 %", "70 - 90 %"],
      answer: "70 - 90 %",
    },
    {
      question:
        "Jaké riziko riziko je třeba zvážit při investování na amerických burzách oproti tuzemským?",
      options: [
        "Riziko likvidity - na amerických burzách je menší objem obchodů než na tuzemských",
        "Riziko krachu americké ekonomiky - ekonomika USA je předlužení a nestabilní a může se snadno zhroutit",
        "Měnové riziko - na americké burze se obchoduje v dolarech, který mění hodnotu vůči koruně",
        "Riziko vysokých poplatků - obchodování na amerických burzách je výrazně dražší než na tuzemských",
      ],
      answer:
        "Měnové riziko - na americké burze se obchoduje v dolarech, který mění hodnotu vůči koruně",
    },
    {
      question: "Má měnové riziko na investici pozitivní nebo negativní vliv?",
      options: [
        "Měnové riziko má vždy negativní vliv",
        "Měnové riziko má vždy pozitivní vliv",
        "Jak kdy, kurz měny se může pohybovat oběma směry",
        "Měnové riziko na investici nemá vliv",
      ],
      answer: "Jak kdy, kurz měny se může pohybovat oběma směry",
    },
    {
      question:
        "Má měnové riziko větší u stabilních měnových párů vliv na krátkodobého nebo dlouhodobého investora?",
      options: [
        "Na krátkodobého investora",
        "Na dlouhodobého investora",
        "Je to jedno, měnové riziko je vždy stejné",
        "Měnové riziko nemá vliv na investora",
      ],
      answer: "Na krátkodobého investora",
    },
    {
      question: "Co je to kapitalizace trhu?",
      options: [
        "Počet investorů investující do daného trhu",
        "Celkové množství peněz investovaných do určitého aktiva",
        "Celkový zisk společnosti za fiskální rok",
        "Výše dividend vyplácených akcionářům",
      ],
      answer: "Celkové množství peněz investovaných do určitého aktiva",
    },
    {
      question: "Co je to index investičních aktiv?",
      options: [
        "Souhrn vybraných investičních aktiv",
        "Měřítko výkonnosti investičního portfolia",
        "Měřítko rizikovosti investičního portfolia",
        "Průměrná cena jednotlivých investičních aktiv na trhu",
      ],
      answer: "Souhrn vybraných investičních aktiv",
    },
  ],
};
